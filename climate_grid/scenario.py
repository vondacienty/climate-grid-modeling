...
def _validate_ensemble_multi(
    ensemble: Any, *, where: str = "ensemble"
    ...
    for element in elements:
        ...
        if len(quantile_values) != len(quantiles):
            raise ValueError(
                f"{series_where}.quantile_values must have {len(quantiles)} frames "
                "(one per quantile)"
            )
...
def value_delta_multi(left, right) -> dict:
    """Compute the right-minus-left delta of two ensemble-multi results.
    ...
    every member is validated against that contract, including the nested
    ``[time][lat][lon]`` ``values``/``status``/``uncertainty`` members, the
    nested ``[quantile][time][lat][lon]`` ``quantile_values`` frames and the
    ``missing`` cell invariants.  The two results must share equal
    ``elements``, ``times``, ``lats``, ``lons`` and ``quantiles`` in the
    same order.

    For every element and every ``[time][lat][lon]`` cell, a cell whose
    status is ``missing`` in either input gets ``values=None``,
    ``status="missing"``, ``uncertainty=None`` and ``None`` for every
    quantile; otherwise ``values`` is ``right - left``, ``status`` is
    ``"observed"`` when both inputs are observed and ``"interpolated"``
    otherwise, and ``uncertainty`` is ``sqrt(u_left ** 2 + u_right ** 2)``.
    Each quantile value is ``right - left`` when both inputs are not
    ``None`` and ``None`` otherwise.

    The returned mapping uses the key order ``schema, elements, times,
    lats, lons, quantiles, data``; ``schema`` is
    ``climate-grid/ensemble-delta-multi-v1`` and the axes and labels echo
    the (shared) input arrays.  ``data`` follows the input element order
    and each item uses the key order ``values, status, uncertainty,
    ...
    for element in left_elements:
        ...
        out_quantiles = [
            ...
            for _ in left_quantiles
        ]

        for t in range(n_times):
            for i in range(n_lat):
                for j in range(n_lon):
                    left_status = left_series["status"][t][i][j]
                    right_status = right_series["status"][t][i][j]
                    if left_status == "missing" or right_status == "missing":
                        out_status[t][i][j] = "missing"
                        continue
                    out_values[t][i][j] = _round_output(
                        right_series["values"][t][i][j]
                        - left_series["values"][t][i][j]
                    )
                    out_status[t][i][j] = (
                        "observed"
                    ...
                    out_uncertainty[t][i][j] = _round_output(
                        math.sqrt(
                            left_series["uncertainty"][t][i][j] ** 2
                            + right_series["uncertainty"][t][i][j] ** 2
                        )
                    )
                    for q_index in range(len(left_quantiles)):
                        left_q = left_series["quantile_values"][q_index][t][i][j]
                        right_q = right_series["quantile_values"][q_index][t][i][j]
