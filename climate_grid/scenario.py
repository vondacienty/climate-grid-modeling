...
from .regional import (
    ...
    _validate_quantiles as _validate_regional_quantiles,
    _validate_regions,
    _validate_windows,
    _window_region,
)
...
_ENSEMBLE_DELTA_QUANTILE_REGION_SCHEMA = "climate-grid/dq-region-v1"
...
def aggregate_value_delta_quantile(
    ...
    ``elements`` and ``windows`` echo the delta elements and the ``windows``
    argument, ``quantiles`` echoes the argument and ``regions`` lists the
    ...
    validated_regions = _validate_regions(regions, len(lats), len(lons))
    validated_windows = _validate_windows(windows, times)
    validated_quantiles = _validate_regional_quantiles(quantiles)
    ...
    return {
        "schema": _ENSEMBLE_DELTA_QUANTILE_REGION_SCHEMA,
        "elements": list(elements),
        "quantiles": quantiles,
        "windows": windows,
        "regions": [name for name, _ in validated_regions],
