...
def summarize_region_comparison(comparison, *, min_scenarios: int = 1) -> dict:
    """Summarize per-window/region/element differences across scenarios.

    ``comparison`` must be a complete :func:`compare_region_report` result
    (schema ``climate-grid/rr-compare-v1``) with exactly the keys ``schema,
    ...
    the non-empty str ``reference`` and ``baseline`` (``baseline`` must
    not appear in ``scenarios``), the non-empty unique str lists
    ``scenarios``, ``elements`` and ``regions``, the ``windows`` entries
    (key order ``name, start, end``), the flat
    scenario-then-window-then-region-then-element ``data`` row order,
    each row's key order ``scenario, window, region, element, difference,
    uncertainty`` and the row invariants (``difference`` and
    ``uncertainty`` both ``None`` or both finite numbers, with
    ``uncertainty`` non-negative).  ``min_scenarios`` must be a non-bool
    positive int.

    For every window (in window order), region (in region order) and
    element (in element order), the ``(difference, uncertainty)`` pairs
    whose members are both not ``None`` are collected in scenario order
    and ``count`` is their number ``n``.  When ``n`` is below
    ``min_scenarios``, ``mean``, ``min``, ``max`` and ``uncertainty`` are
    all ``None``; otherwise they are ``sum(d) / n``, the smallest ``d``,
    the largest ``d`` and ``sqrt(sum(u ** 2)) / n`` respectively.

    The returned mapping uses the key order ``schema, years, reference,
    baseline, scenarios, elements, windows, regions, data``; ``schema``
    is ``climate-grid/rr-summary-v1`` and ``years``, ``reference``,
    ``baseline``, ``scenarios``, ``elements``, ``windows`` and
    ``regions`` echo the input metadata in their original order.
    ``data`` is a flat list in window-then-region-then-element order;
    each row uses the key order ``window, region, element, count, mean,
    min, max, uncertainty``.  ``count`` is an int and every output float
