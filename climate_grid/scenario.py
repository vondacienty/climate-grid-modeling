...
def _validate_rank_transitions(
    ...
    n_scenarios = len(scenarios)
    expected_rows = n_configs * n_scenarios
    if len(data) != expected_rows:
        raise ValueError(
            f"{where}.data must have {expected_rows} rows "
    ...
    for config in configs:
        for scenario in scenarios:
            ...
            if baseline_rank is None or rank is None:
                ...
                if delta is None:
                    raise ValueError(
                        f"{row_where}.delta must be present when both ranks "
                        "are present"
                    )
    ...
    for index, group in enumerate(transitions):
        ...
        if tuple(group.keys()) != _RANK_TRANSITIONS_GROUP_KEYS:
            raise ValueError(
                f"{group_where} must have exactly the keys baseline_rank, "
                "rank, count in order"
            )
        for name in ("baseline_rank", "rank", "count"):
            value = group[name]
            if not isinstance(value, int) or isinstance(value, bool):
                raise TypeError(f"{group_where}.{name} must be a non-bool int")
            if value < 1:
                raise ValueError(f"{group_where}.{name} must be positive")
        transitions_total += group["count"]
    if transitions_total != valid_rows:
        raise ValueError(
...
def rank_transition_summary(result) -> dict:
    """Summarize rank transitions per perturbation configuration.
    ...
    and ``None`` otherwise) and the ``transitions`` counts summing to the
    number of data rows with both ranks present.

    For every configuration (in configuration order), ``total`` is the
    number of scenarios, ``valid`` is the number of the configuration's
    ...
    ``stable / valid``.  The configuration's rows with both ranks present
    are also counted by their ``(baseline_rank, rank)`` pair; the groups
    are sorted by the pair in ascending order and each group's ``rate`` is
    ``count / valid``.
    ...
    configuration-then-group order; each row uses the key order ``config,
    ...
    for c_index, config in enumerate(configs):
        rows = data[c_index * n_scenarios:(c_index + 1) * n_scenarios]
        valid_rows = [
            row
            for row in rows
