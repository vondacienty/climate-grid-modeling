...
_CONVERGE_MATRIX_RESULT_KEYS = ("schema", "tolerance", "layers", "data")
...
def _validate_converge_matrix(
    matrix: Any, *, where: str = "matrix"
) -> tuple[float, list, list[str], list]:
    """Validate a complete :func:`converge_matrix` result.

    Returns the tolerance, the ``layers`` pairs, the scenario order and
    the ``data`` rows.
    """
    if not isinstance(matrix, dict):
        raise TypeError(f"{where} must be a dict")
    if tuple(matrix.keys()) != _CONVERGE_MATRIX_RESULT_KEYS:
        raise ValueError(
            f"{where} must have exactly the keys schema, tolerance, "
            "layers, data in order"
        )
    ...
    for index, row in enumerate(data):
        ...
        if tuple(states.keys()) != _CONVERGE_STATES:
            raise ValueError(
        ...
        for state in _CONVERGE_STATES:
            ...
            if not isinstance(state_count, int) or isinstance(state_count, bool):
                raise TypeError(
                    f"{row_where}.states.{state} must be a non-bool int"
                )
            if state_count < 0:
                raise ValueError(
                    f"{row_where}.states.{state} must be non-negative"
                )
            state_total += state_count
        if state_total != count:
            raise ValueError(
                f"{row_where}.states counts must sum to count ({count})"
            )
        ...
        if not isinstance(interval, list):
            raise TypeError(f"{row_where}.interval must be a list")
        if len(interval) != 2:
            raise ValueError(f"{row_where}.interval must have exactly 2 bounds")
        for bound_index, bound in enumerate(interval):
            _validate_number(
                bound, f"{row_where}.interval[{bound_index}]", nullable=False
            )
    ...
    return tolerance, layers, scenarios, data


def compare_matrix(items) -> dict:
    """Compare scenario converge matrices against a reference matrix.

    ``items`` must be a list of at least two mappings, each with exactly
    the keys ``name, matrix`` in that order.  ``name`` is a unique
    non-empty str and ``matrix`` is a complete :func:`converge_matrix`
    result (schema ``climate-grid/cvm-v1``); every member is validated
    against that contract.  Every matrix must share the same
    ``tolerance``, ``layers`` and scenario order; the shared axes are
    taken from the first item.

    The first item is the reference and every remaining item is compared
    against it per scenario (in scenario order): ``count`` is the item's
    ``count`` minus the reference's; ``states`` is a list of three count
    differences ordered ``stable, uncertain, divergent``; when both
    rows' ``interval`` is present, ``interval`` is
    ``[item.lo − reference.lo, item.hi − reference.hi]`` and otherwise
    ``None``; ``from`` and ``to`` take the reference's and the item's
    ``state`` respectively (either may be ``None``); when both rows'
    ``rank`` is present, ``rank`` is the item's rank minus the
    reference's and otherwise ``None``.

    The returned mapping uses the key order ``schema, reference, names,
    tolerance, layers, scenarios, data``; ``schema`` is
    ``climate-grid/cmc-v1``, ``reference`` is the first item's name and
    ``names`` lists the remaining item names in item order.  The
    tolerance, layers and scenarios axes echo the first matrix.  ``data``
    is ordered by item name (the remaining items in order) and then
    scenario; each row uses the key order ``name, scenario, count,
    states, interval, from, to, rank``.  ``count``, the ``states``
    differences and ``rank`` are ints and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  The
    input is not modified.
    ...
    for index, item in enumerate(items):
        ...
        tolerance, layers, scenarios, data = _validate_converge_matrix(
        ...
        if not validated:
            ...
            if tolerance != first_tolerance:
                raise ValueError(
                    "all items must share the same matrix tolerance, taken "
                    "from the first item"
                )
            if layers != first_layers:
                raise ValueError(
                    "all items must share the same matrix layers in the same "
                    "order, taken from the first item"
                )
            if scenarios != first_scenarios:
                raise ValueError(
                    "all items must share the same matrix scenarios in the "
                    "same order, taken from the first item"
                )
    ...
    reference_rows = validated[0]

    rows = []
    for name, lookup in zip(compare_names, validated[1:]):
        for scenario in first_scenarios:
            base = reference_rows[scenario]
            ...
            if base_interval is None or current_interval is None:
                interval = None
            else:
                interval = [
                    _round_output(current_interval[0] - base_interval[0]),
                    _round_output(current_interval[1] - base_interval[1]),
                ]
            base_rank = base["rank"]
            current_rank = current["rank"]
            if base_rank is None or current_rank is None:
                rank = None
            else:
