...
def layer_trend(summaries, years, *, min_points: int = 2) -> dict:
    ...
    rows with ``count`` equal to ``0`` must have all four ``None``).
        ...
        for element in elements:
            for driver in drivers:
                ...
                if tuple(row.keys()) != _LAYER_TREND_ROW_KEYS:
                    raise ValueError(
                        f"{row_where} must have exactly the keys scenario, "
                        "element, driver, count, slope, uncertainty in order"
                    )
                ...
                if (slope is None) != (uncertainty is None):
                    raise ValueError(
                        f"{row_where}: slope and uncertainty must be both "
...
def compare_trends(trend) -> dict:
    """Compare :func:`layer_trend` scenario trends against the first scenario.

    ``trend`` must be a complete :func:`layer_trend` result (schema
    ``climate-grid/lt-v1``); it is validated against that contract,
    including the key order ``schema, years, reference, scenarios,
    elements, drivers, data``, the strictly increasing non-bool int
    ``years``, the flat scenario-then-element-then-driver ``data`` row
    order, each row's key order ``scenario, element, driver, count,
    slope, uncertainty`` and the row invariants (``count`` a
    non-negative non-bool int; ``slope`` and ``uncertainty`` both
    ``None`` or both finite numbers, with ``uncertainty`` non-negative).
    ``scenarios`` must contain at least two items.

    The first scenario is the baseline.  For every remaining scenario
    (in scenario order), element (in element order) and driver (in
    driver order), its row is paired with the baseline row for the same
    element and driver and ``count`` is the smaller of the two row
    counts.  When either row's count is below ``2`` or either row's
    ``slope`` or ``uncertainty`` is ``None``, ``difference`` and
    ``uncertainty`` are both ``None``; otherwise ``difference`` is the
    scenario slope minus the baseline slope and ``uncertainty`` is
    ``sqrt(u0 ** 2 + u1 ** 2)`` with ``u0`` the baseline and ``u1`` the
    scenario uncertainty.

    The returned mapping uses the key order ``schema, years, reference,
    scenarios, elements, drivers, data``; ``schema`` is
    ``climate-grid/ltc-v1``, ``years``, ``elements`` and ``drivers``
    echo the input in their original order, ``reference`` is the first
    scenario and ``scenarios`` lists the remaining scenarios.  ``data``
    is a flat list in scenario-then-element-then-driver order; each row
    uses the key order ``scenario, element, driver, count, difference,
    uncertainty``.  ``count`` is a non-bool int and every output float
    ...
    ``ValueError`` for any other contract violation.
    """
    years, _reference, scenarios, elements, drivers, data = (
    ...
    result_data = []
    for s_index, scenario in enumerate(remaining, start=1):
        for e_index, element in enumerate(elements):
            for d_index, driver in enumerate(drivers):
                base_row = data[(0 * n_elements + e_index) * n_drivers + d_index]
                row = data[(s_index * n_elements + e_index) * n_drivers + d_index]

                base_slope = base_row["slope"]
                base_uncertainty = base_row["uncertainty"]
                slope = row["slope"]
                uncertainty = row["uncertainty"]
                count = min(base_row["count"], row["count"])
                if (
                    count < 2
                    or base_slope is None
                    ...
                    difference = _round_output(slope - base_slope)
                    combined = _round_output(
                        math.sqrt(
