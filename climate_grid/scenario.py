...
def aggregate_value_delta_entropy(
    ...
    element (in delta element order), the non-missing ``(value,
    uncertainty)`` samples of the closed calendar interval are collected
    across the region's cells.  With ``B = len(edges) + 1`` bins, samples
    are binned exactly as in :func:`aggregate_value_delta_histogram`;
    ``count`` is the sample count ``n`` and ``bin_counts`` lists the ``B``
    int bin counts.  When ``n`` is below ``min_count``, ``entropy`` and
    ``uncertainty`` are both ``None``.  Otherwise, with
    ``p = bin_counts[i] / n``, ``entropy`` is ``-sum(p * log2(p))`` over
    the bins with ``p > 0`` and ``uncertainty`` is ``sqrt(sum(u ** 2)) / n``
    over the sampled uncertainties.
