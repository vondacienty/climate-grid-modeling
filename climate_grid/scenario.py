...
_REGION_RANK_STABILITY_SCHEMA = "climate-grid/rrs-v1"
_REGION_RANK_STABILITY_RESULT_KEYS = ("schema", "scenarios", "data")
_REGION_RANK_STABILITY_ROW_KEYS = (
    "scenario",
    "total",
    "valid",
    "missing_rate",
...
def _validate_region_interval_ranking(
    ranking: Any, *, where: str = "ranking"
) -> tuple[list[str], list[str], list, list, list]:
    """Validate a complete :func:`rank_region_intervals` result."""
    if not isinstance(ranking, dict):
        raise TypeError(f"{where} must be a dict")
    if tuple(ranking.keys()) != _REGION_REPORT_INTERVAL_RANK_RESULT_KEYS:
        raise ValueError(
            f"{where} must have exactly the keys schema, scenarios, elements, "
            "windows, regions, factors, pairs, data in order"
        )
    ...
    scenarios = _validate_string_list(ranking["scenarios"], f"{where}.scenarios")
    elements = _validate_string_list(ranking["elements"], f"{where}.elements")
    ...
    for index, window in enumerate(windows):
        ...
        if window_name in seen_windows:
            raise ValueError(f"duplicate {where}.windows name: {window_name!r}")
        seen_windows.add(window_name)
        start_day = _parse_date(window["start"], f"{window_where}.start")
        end_day = _parse_date(window["end"], f"{window_where}.end")
        if start_day > end_day:
            raise ValueError(
                f"{window_where}.start must be on or before {window_where}.end"
            )
    ...
    for index, pair in enumerate(pairs):
        ...
        if list(pair.keys()) != ["name", "left", "right"]:
            raise ValueError(
                f"{pair_where} must have exactly the keys name, left, right "
                "in order"
            )
        pair_name = pair["name"]
        left = pair["left"]
        ...
        if left == right:
            raise ValueError(
                f"{pair_where}.left and {pair_where}.right must be different "
                "regions"
            )
    ...
    if not isinstance(data, list):
        raise TypeError(f"{where}.data must be a list")
    n_scenarios = len(scenarios)
    expected_rows = len(pairs) * len(elements) * len(factors) * n_scenarios
    if len(data) != expected_rows:
        raise ValueError(
            f"{where}.data must have {expected_rows} rows (one per "
            "pair/element/factor/scenario combination, in pair-then-element-"
            "then-factor-then-rank order)"
        )
    ...
    for pair in pairs:
        for element in elements:
            for factor in factors:
                group_where = (
                    f"{where}.data group "
                    f"({pair['name']!r}, {element!r}, {factor!r})"
                )
                seen_scenarios: set[str] = set()
                valid_count = 0
                null_phase = False
                ...
                for _ in range(n_scenarios):
                    ...
                    if tuple(row.keys()) != _REGION_REPORT_INTERVAL_RANK_ROW_KEYS:
                        raise ValueError(
                            ...
                            "center_delta, lower_delta, upper_delta, overlap, "
                            "rank in order"
                        )
                    ...
                    for key, expected in (
                        ...
                        if value != expected:
                            raise ValueError(
                                f"{row_where}.{key} must be {expected!r} for "
                                "its pair-then-element-then-factor-then-rank "
                                "position"
                            )
                    ...
                    if row_factor != factor:
                        raise ValueError(
                            f"{row_where}.factor must be {factor!r} for its "
                            "pair-then-element-then-factor-then-rank position"
                        )
                    ...
                    if row_scenario not in scenario_index:
                        raise ValueError(
                            f"{row_where}.scenario {row_scenario!r} must appear "
                            f"in {where}.scenarios"
                        )
                    if row_scenario in seen_scenarios:
                        raise ValueError(
                            f"{group_where}: scenario {row_scenario!r} appears "
                            "more than once"
                        )
                    seen_scenarios.add(row_scenario)
                    scenario_order = scenario_index[row_scenario]
                    ...
                    if rank is None:
                        ...
                        if any(present):
                            raise ValueError(
                                f"{row_where}: center_delta, lower_delta, "
                                "upper_delta and overlap must be None when "
                                "rank is None"
                            )
                        if scenario_order <= invalid_order:
                            raise ValueError(
                        ...
                        if (
                            ...
                            raise ValueError(
                                f"{row_where}: tied center_delta values must "
                                "keep the original scenario order"
                            )
                        previous_order = scenario_order
    ...
    return scenarios, elements, factors, pairs, data


def rank_region_stability(ranking, *, min_ranks: int = 1) -> dict:
    """Aggregate scenario ranks across pair/element/factor comparisons.
    ...
    data`` in that order; every member is validated against that
    contract, including the flat pair-then-element-then-factor-then-rank
    ``data`` order, each row's key order ``pair, left, right, element,
    factor, scenario, center_delta, lower_delta, upper_delta, overlap,
    rank`` and the row invariants (ranked rows run consecutively from 1,
    precede the ``rank`` ``None`` rows and are ordered by descending
    ``center_delta`` with ties keeping the original scenario order;
    ``center_delta``, ``lower_delta`` and ``upper_delta`` are finite
    numbers and ``overlap`` a bool exactly when ``rank`` is not
    ``None``; the unranked rows keep the original scenario order; every
    scenario appears exactly once per pair/element/factor group).
    ``min_ranks`` must be a non-bool positive int.

    Let ``total = len(pairs) * len(elements) * len(factors)``.  For each
    scenario (in scenario order), its non-``None`` ranks are gathered
    across pairs, elements and factors; their number is ``valid`` and
    ``missing_rate = (total - valid) / total``.  When
    ``valid < min_ranks``, ``mean``, ``best``, ``worst`` and
    ``stddev`` are all ``None``; otherwise they are the mean, minimum
    and maximum of the gathered ranks and
    ``stddev = sqrt(sum((rank - mean) ** 2) / valid)``.

    The returned mapping uses the key order ``schema, scenarios, data``;
    ``schema`` is ``climate-grid/rrs-v1`` and ``scenarios`` echoes the
    ranking's scenario names.  ``data`` follows the scenario order; each
    row uses the key order ``scenario, total, valid, missing_rate, mean,
    best, worst, stddev``.  ``total`` and ``valid`` are ints, ``best``
    and ``worst`` are positive non-bool ints or ``None``, and the other
    statistics are floats or ``None``; every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  The
    input is not modified.
    ...
    scenarios, elements, factors, pairs, data = (
        _validate_region_interval_ranking(ranking)
    )
...
def _validate_window_rank(
    ...
    for index, row in enumerate(data):
        ...
        if row["missing_rate"] != _round_output((total - valid) / total):
            raise ValueError(
                f"{row_where}.missing_rate must equal "
                "(total - valid) / total"
            )

        stat_fields = ("mean", "best", "worst", "stddev")
        _validate_number(row["mean"], f"{row_where}.mean", nullable=True)
        _validate_number(row["stddev"], f"{row_where}.stddev", nullable=True)
        ...
        if not any(none_flags):
            ...
            if not (row["best"] <= row["mean"] <= row["worst"]):
                raise ValueError(
                    f"{row_where}.mean must lie between best and worst"
                )
            if row["stddev"] < 0:
                raise ValueError(f"{row_where}.stddev must be non-negative")
    ...
    n_rows = len(data)
    while index < n_rows:
        window = data[index]["window"]
        if window in seen_windows:
            raise ValueError(
                f"{where}.data rows for window {window!r} must form a "
                "single contiguous block"
            )
        seen_windows.add(window)
        block_scenarios = []
        block_means = []
        while index < n_rows and data[index]["window"] == window:
            block_scenarios.append(data[index]["scenario"])
            block_means.append(data[index]["mean"])
            index += 1
        if len(set(block_scenarios)) != len(block_scenarios):
            raise ValueError(
                f"{where}.data window {window!r} lists a scenario more "
                "than once"
            )
        if scenario_order is None:
            scenario_order = block_scenarios
        elif block_scenarios != scenario_order:
            raise ValueError(
                f"{where}.data window {window!r} must list the same "
                "scenarios in the same order as the first window"
            )
        windows.append(window)
        means.append(block_means)

    return windows, scenario_order, means


def rank_change(result, *, min_windows: int = 2) -> dict:
    """Measure per-scenario rank-mean changes across ordered windows.

    ``result`` must be a complete :func:`window_rank` result (schema
    ``climate-grid/wr-v1``) whose ``data`` spans at least two windows,
    arranged window-then-scenario with distinct window names and the
    same scenario order in every window.  ``min_windows`` must be a
    non-bool positive int.

    For each scenario (in scenario order) the per-window ``mean`` values
    form a series over the windows (in data order).  ``changes`` holds,
    for each adjacent window pair, the later mean minus the earlier one,
    or ``None`` when either mean is ``None``.  ``valid`` and ``count``
    are the numbers of non-``None`` means and non-``None`` changes.
    When ``valid < min_windows`` or ``count == 0``, ``mean_abs`` and
    ``max`` are both ``None``; otherwise they are the mean and the
    maximum of the absolute changes.  ``slope`` is the OLS slope of the
    non-``None`` means against their window positions (0-based); it is
    ``None`` when ``valid < max(min_windows, 2)``.

    The returned mapping uses the key order ``schema, windows,
    scenarios, data``; ``schema`` is ``climate-grid/wrc-v1`` and both
    axes follow the input order.  ``data`` follows the scenario order;
    each row uses the key order ``scenario, valid, count, changes,
    mean_abs, max, slope`` and ``changes`` has ``len(windows) - 1``
    members.  ``valid`` and ``count`` are ints; the ``changes`` members
    and the three statistics are floats or ``None``; every output float
    ...
    ``ValueError`` for any other contract violation.
    """
    windows, scenarios, means = _validate_window_rank(result)
    if len(windows) < 2:
    ...
    result_rows = []
    for s_index, scenario in enumerate(scenarios):
        series = [means[w_index][s_index] for w_index in range(n_windows)]
        valid = sum(1 for mean in series if mean is not None)

        changes = []
        for w_index in range(n_windows - 1):
            left = series[w_index]
            right = series[w_index + 1]
            if left is None or right is None:
                changes.append(None)
            else:
                changes.append(_round_output(right - left))
        ...
        if valid < min_windows or count == 0:
            ...
            mean_abs = _round_output(
                sum(abs(change) for change in present) / count
            )
            maximum = _round_output(max(abs(change) for change in present))
        ...
        if valid < slope_threshold:
            ...
            xs = [
                w_index
                for w_index in range(n_windows)
                if series[w_index] is not None
            ]
            ys = [series[w_index] for w_index in xs]
            x_mean = sum(xs) / valid
            y_mean = sum(ys) / valid
...
_REGION_WINDOW_RANK_CHANGE_STABILITY_RESULT_KEYS = (
    "schema",
    "windows",
    "scenarios",
    "data",
)
_REGION_WINDOW_RANK_CHANGE_STABILITY_ROW_KEYS = (
    "scenario",
    "valid",
    "count",
    "changes",
...
def _validate_rank_change(
    ...
    n_changes = len(windows) - 1
    for index, row in enumerate(data):
        row_where = f"{where}.data[{index}]"
        if not isinstance(row, dict):
            raise TypeError(f"{row_where} must be a dict")
        if tuple(row.keys()) != _REGION_WINDOW_RANK_CHANGE_STABILITY_ROW_KEYS:
            raise ValueError(
        ...
        if len(changes) != n_changes:
            raise ValueError(
                f"{row_where}.changes must have {n_changes} members "
        ...
        if count > max(valid - 1, 0):
            raise ValueError(
                f"{row_where}.count must not exceed {row_where}.valid - 1"
            )
        ...
        if (mean_abs is None) != (maximum is None):
            raise ValueError(
                f"{row_where}: mean_abs and max must be both None or both "
                "present"
            )
        ...
        slope = row["slope"]
        _validate_number(slope, f"{row_where}.slope", nullable=True)
        if valid < 2 and slope is not None:
            raise ValueError(
                f"{row_where}.slope must be None when valid is below two"
            )
...
def rank_change_stability(result, *, min_changes: int = 1) -> dict:
    """Summarize per-scenario rank-change stability across ordered windows.

    ``result`` must be a complete :func:`rank_change` result (schema
    ``climate-grid/wrc-v1``) with exactly the keys ``schema, windows,
    scenarios, data`` in that order; every member is validated against
    that contract, including each row's key order ``scenario, valid,
    count, changes, mean_abs, max, slope`` and the count / mean_abs /
    max / slope invariants.  ``min_changes`` must be a non-bool positive
    int.

    For each scenario (in scenario order) ``count`` is the number of
    non-``None`` members of ``changes``.  ``reversals`` counts adjacent
    ``changes`` pairs that are both non-``None`` and non-zero with
    opposite signs.  ``longest_run`` is the length of the longest run of
    consecutive non-zero same-sign changes; ``None`` and zero members
    interrupt a run.

    When ``count < min_changes``, ``reversals``, ``longest_run``,
    ``stable_rate`` and ``level`` are all ``None``.  Otherwise
    ``stable_rate`` is the number of zero changes divided by ``count``
    and the score is
    ``S = mean(abs(changes)) * (1 + reversals / max(count - 1, 1))``;
    ``level`` is ``stable`` for ``S == 0``, ``low`` for
    ``0 < S <= 1``, ``medium`` for ``1 < S <= 2`` and ``high`` for
    ``S > 2``.

    The returned mapping uses the key order ``schema, windows,
    scenarios, data``; ``schema`` is ``climate-grid/wrc-stability-v1``
    and both axes echo the input order.  ``data`` follows the scenario
    order; each row uses the key order ``scenario, count, reversals,
    longest_run, stable_rate, level``.  ``count``, ``reversals`` and
    ``longest_run`` are ints (or ``None``) and ``stable_rate`` is a
    float or ``None``; every output float is ``round(x, 12)`` with
    negative zero normalized to ``0.0``.  The input is not modified.

    Raises ``TypeError`` for wrong container/item/argument types and
    ``ValueError`` for any other contract violation.
    """
    windows, scenarios, data = _validate_rank_change(result)
    ...
    for row in data:
        ...
        if count < min_changes:
            ...
            score = mean_abs * (1 + reversals / max(count - 1, 1))
            if score == 0:
                level = "stable"
            elif score <= 1:
                level = "low"
...
_SCALE_DELTA_RESULT_KEYS = ("schema", "windows", "scenarios", "items", "data")
...
_SCALE_SUMMARY_SCHEMA = "climate-grid/ss-v1"
_SCALE_SUMMARY_RESULT_KEYS = ("schema", "scales", "scenarios", "data")
_SCALE_SUMMARY_ROW_KEYS = (
    "left",
    "right",
    "scenario",
    "missing_delta",
...
def _validate_stability_sum(
    ...
    if schema != _STABILITY_SUM_SCHEMA:
        raise ValueError(
            f"{where}.schema must be {_STABILITY_SUM_SCHEMA!r}"
        )
    ...
    scenarios = _validate_string_list(result["scenarios"], f"{where}.scenarios")
    items = _validate_string_list(result["items"], f"{where}.items")
    ...
    for index, row in enumerate(data):
        ...
        if not isinstance(row, dict):
            raise TypeError(f"{row_where} must be a dict")
        if tuple(row.keys()) != _SCALE_DELTA_ROW_KEYS:
            raise ValueError(
        ...
        if total != len(items):
            raise ValueError(
                f"{row_where}.total must equal the number of items"
            )
        ...
        _validate_number(row["missing"], f"{row_where}.missing", nullable=False)
        if not 0 <= row["missing"] <= 1:
            raise ValueError(f"{row_where}.missing must be between 0 and 1")
        if row["missing"] != round((total - valid) / total, 12):
            raise ValueError(
                f"{row_where}.missing must equal "
                "round((total - valid) / total, 12)"
            )
        ...
        for level_index, count in enumerate(levels):
            if not isinstance(count, int) or isinstance(count, bool):
                raise TypeError(
                    f"{row_where}.levels[{level_index}] must be a non-bool int"
                )
            if count < 0:
                raise ValueError(
                    f"{row_where}.levels[{level_index}] must be non-negative"
                )
        if sum(levels) != valid:
            raise ValueError(
                f"{row_where}.levels must sum to {row_where}.valid"
            )
        ...
        if rate is None or uncertainty is None:
            if rate is not None or uncertainty is not None:
                raise ValueError(
                    f"{row_where}: rate and uncertainty must be both None "
                    "or both present"
                )
            continue
        ...
        if valid == 0:
            raise ValueError(
                f"{row_where}: rate and uncertainty must be None when "
                "valid is zero"
            )
        _validate_number(rate, f"{row_where}.rate", nullable=False)
        if not 0 <= rate <= 1:
            raise ValueError(f"{row_where}.rate must be between 0 and 1")
        _validate_number(
            uncertainty, f"{row_where}.uncertainty", nullable=False
        )
        if uncertainty < 0:
    ...
    return windows, scenarios, items, data


def scale_delta(items) -> dict:
    """Compute per-scenario deltas between adjacent named stability scales.

    ``items`` must be a list of at least two mappings, each with exactly
    the keys ``name, result`` in that order.  ``name`` is a unique
    non-empty str and ``result`` is a complete :func:`stability_sum`
    result (schema ``climate-grid/ws-v1``); every member is validated
    against that contract.  Every result must share the same ``windows``,
    ``scenarios`` and ``items`` in the same order; ``windows`` and
    ``scenarios`` are taken from the first item.

    For every adjacent pair of items (in item order) and every scenario
    (in scenario order) a row is emitted: ``missing_delta`` is the later
    row's ``missing`` minus the earlier row's, ``level_deltas`` is the
    later row's ``levels`` minus the earlier row's elementwise (four
    ints), and only when both rows' ``rate`` and ``uncertainty`` are not
    ``None`` are ``rate_delta`` (later ``rate`` minus earlier ``rate``)
    and ``uncertainty`` (``hypot`` of the two uncertainties) computed;
    ...
    result.  ``data`` follows the adjacent-pair-then-scenario order;
    each row uses the key order ``left, right, scenario, missing_delta,
    level_deltas, rate_delta, uncertainty`` with ``left``/``right`` the
    earlier/later item names.  Every output float is ``round(x, 12)``
    with negative zero normalized to ``0.0``.  The input is not
    modified.
    ...
    for index, item in enumerate(items):
        ...
        windows, scenarios, members, data = _validate_stability_sum(
            item["result"], where=f"{where}.result"
        )
        if not validated:
            first_windows = windows
    ...
    for index in range(len(validated) - 1):
        left_data = validated[index]
        right_data = validated[index + 1]
        for s_index, scenario in enumerate(first_scenarios):
            left = left_data[s_index]
            right = right_data[s_index]
            missing_delta = _round_output(right["missing"] - left["missing"])
            level_deltas = [
            ...
            if (
                ...
                or right["rate"] is None
                or right["uncertainty"] is None
            ):
                rate_delta = None
                uncertainty = None
            else:
                rate_delta = _round_output(right["rate"] - left["rate"])
                uncertainty = _round_output(
...
def _validate_scale_delta(
    result: Any, *, where: str = "result"
) -> tuple[list[str], list[str], list]:
    """Validate a complete :func:`scale_delta` result.

    Returns the scale names, the scenario order and the ``data`` rows.
    """
    if not isinstance(result, dict):
        raise TypeError(f"{where} must be a dict")
    if tuple(result.keys()) != _SCALE_SUMMARY_RESULT_KEYS:
        raise ValueError(
    ...
    if not isinstance(schema, str):
        raise TypeError(f"{where}.schema must be a str")
    if schema != _SCALE_DELTA_SCHEMA:
        raise ValueError(f"{where}.schema must be {_SCALE_DELTA_SCHEMA!r}")
    ...
    if not isinstance(data, list):
        raise TypeError(f"{where}.data must be a list")
    expected = (len(scales) - 1) * len(scenarios)
    if len(data) != expected:
        raise ValueError(
            f"{where}.data must have {expected} rows (one per adjacent "
            "scale pair and scenario, in pair-then-scenario order)"
        )
    ...
    for index, row in enumerate(data):
        ...
        if not isinstance(row, dict):
            raise TypeError(f"{row_where} must be a dict")
        if tuple(row.keys()) != _SCALE_SUMMARY_ROW_KEYS:
            raise ValueError(
                f"{row_where} must have exactly the keys left, right, "
                "scenario, missing_delta, level_deltas, rate_delta, "
                "uncertainty in order"
            )

        pair_index, s_index = divmod(index, len(scenarios))

        left = row["left"]
        if not isinstance(left, str):
            raise TypeError(f"{row_where}.left must be a str")
        if left != scales[pair_index]:
            raise ValueError(
                f"{row_where}.left must be {scales[pair_index]!r} for its "
                "pair-then-scenario position"
            )

        right = row["right"]
        if not isinstance(right, str):
            raise TypeError(f"{row_where}.right must be a str")
        if right != scales[pair_index + 1]:
            raise ValueError(
                f"{row_where}.right must be {scales[pair_index + 1]!r} for "
                "its pair-then-scenario position"
            )
        ...
        if scenario != scenarios[s_index]:
            raise ValueError(
                f"{row_where}.scenario must be {scenarios[s_index]!r} for "
                "its pair-then-scenario position"
            )

        _validate_number(
            row["missing_delta"], f"{row_where}.missing_delta", nullable=False
        )
        if not -1 <= row["missing_delta"] <= 1:
            raise ValueError(
                f"{row_where}.missing_delta must be between -1 and 1"
            )

        level_deltas = row["level_deltas"]
        if not isinstance(level_deltas, list):
            raise TypeError(f"{row_where}.level_deltas must be a list")
        if len(level_deltas) != len(_STABILITY_LEVELS):
            raise ValueError(
                f"{row_where}.level_deltas must have "
                f"{len(_STABILITY_LEVELS)} deltas (stable, low, medium, high)"
            )
        for level_index, delta in enumerate(level_deltas):
            if not isinstance(delta, int) or isinstance(delta, bool):
                raise TypeError(
                    f"{row_where}.level_deltas[{level_index}] must be a "
                    "non-bool int"
                )

        rate_delta = row["rate_delta"]
        uncertainty = row["uncertainty"]
        if rate_delta is None or uncertainty is None:
            if rate_delta is not None or uncertainty is not None:
                raise ValueError(
                    f"{row_where}: rate_delta and uncertainty must be both "
                    "None or both present"
                )
            continue

        _validate_number(
            rate_delta, f"{row_where}.rate_delta", nullable=False
        )
    ...
    return scales, scenarios, data


def scale_summary(result, *, min_pairs: int = 1) -> dict:
    """Summarize per-scenario scale deltas across adjacent scale pairs.

    ``result`` must be a complete :func:`scale_delta` result (schema
    ``climate-grid/wsd-v1``); it is validated against that contract.
    ``min_pairs`` must be a non-bool positive int.

    For every scenario (in scenario order) the rows whose ``rate_delta``
    and ``uncertainty`` are both not ``None`` are collected in scale
    order and ``count`` is their number.  When ``count < min_pairs``,
    ``delta``, ``pair``, ``peak`` and ``uncertainty`` are all ``None``;
    otherwise ``delta`` is the sum of the collected rows' ``rate_delta``,
    ``peak`` is the largest absolute ``rate_delta``, ``pair`` is the
    ``[left, right]`` of the first row in scale order reaching it and
    ``uncertainty`` is ``math.hypot`` of the collected uncertainties.

    The returned mapping uses the key order ``schema, scales, data``;
    ``schema`` is ``climate-grid/ss-v1`` and ``scales`` echoes the
    result's scales.  ``data`` follows the scenario order; each row uses
    the key order ``scenario, count, delta, pair, peak, uncertainty``.
    ``count`` is an int and every output float is ``round(x, 12)`` with
    negative zero normalized to ``0.0``.  The input is not modified.

    Raises ``TypeError`` for wrong result/argument types and
    ``ValueError`` for any other contract violation.
    """
    scales, scenarios, data = _validate_scale_delta(result)
    if not isinstance(min_pairs, int) or isinstance(min_pairs, bool):
        raise TypeError("min_pairs must be a non-bool int")
    if min_pairs < 1:
    ...
    for s_index, scenario in enumerate(scenarios):
        present = []
        for pair_index in range(len(scales) - 1):
            row = data[pair_index * len(scenarios) + s_index]
            if row["rate_delta"] is not None and row["uncertainty"] is not None:
                present.append(row)
        count = len(present)
        ...
        if count < min_pairs:
            ...
            delta = _round_output(
                sum(row["rate_delta"] for row in present)
            )
            peak_value = max(abs(row["rate_delta"]) for row in present)
            peak = _round_output(peak_value)
            for row in present:
                if abs(row["rate_delta"]) == peak_value:
                    pair = [row["left"], row["right"]]
                    break
            uncertainty = _round_output(
                math.hypot(*(row["uncertainty"] for row in present))
            )
...
def _validate_converge(
    result: Any, *, where: str = "result"
) -> tuple[float, float, list]:
    """Validate a complete :func:`converge` result.

    Returns the tolerance, the factor and the ``data`` rows.
    """
    if not isinstance(result, dict):
        raise TypeError(f"{where} must be a dict")
    ...
    for index, row in enumerate(data):
        ...
        if not isinstance(row, dict):
            raise TypeError(f"{row_where} must be a dict")
        if tuple(row.keys()) != _CONVERGE_ROW_KEYS:
            raise ValueError(
                f"{row_where} must have exactly the keys scenario, count, "
        ...
        pair = row["pair"]
        peak = row["peak"]
        if pair is None or peak is None:
            if pair is not None or peak is not None:
                raise ValueError(
                    f"{row_where}: pair and peak must be both None or both "
                    "present"
                )
        else:
            if not isinstance(pair, list):
            ...
            for pair_index, name in enumerate(pair):
                if not isinstance(name, str):
                    raise TypeError(
                        f"{row_where}.pair[{pair_index}] must be a str"
                    )
            if (pair[0], pair[1]) not in adjacent_pairs:
                raise ValueError(
                    f"{row_where}.pair must be an adjacent pair of scales"
        ...
        if interval is None or state is None or rank is None:
            if interval is not None or state is not None or rank is not None:
                raise ValueError(
                    f"{row_where}: interval, state and rank must be all None "
                    "or all present"
                )
            continue
        ...
        if interval[0] > interval[1]:
            raise ValueError(
                f"{row_where}.interval lower bound must not exceed the upper "
                "bound"
            )
...
def converge_matrix(items, *, minimum: int = 1) -> dict:
    ...
    ``items`` must be a non-empty list of mappings, each with exactly the
    keys ``element, region, result`` in that order.  ``element`` and
    ``region`` are non-empty strs whose combination is unique across
    items and ``result`` is a complete :func:`converge` result (schema
    ``climate-grid/cv1``); every member is validated against that
    contract.  Every item's result must share the same ``tolerance`` and
    ``factor`` and the same set of scenarios; the scenario order is taken
    from the first item's ``data``.  ``minimum`` must be a non-bool
    positive int.

    For every scenario (in the first item's ``data`` order) the layers
    whose ``state`` and ``interval`` are both not ``None`` are collected
    across items: ``count`` is their number ``n`` and ``states`` counts
    the collected layers at each of the states ``stable, uncertain,
    divergent`` in that order.  When ``n < minimum``, ``interval``,
    ``state`` and ``rank`` are all ``None``; otherwise ``interval`` is
    ``[Σlo / n, Σhi / n]`` over the collected intervals and the state
    follows the :func:`converge` rule against the shared tolerance —
    ``stable`` when the mean ``hi`` is at most the tolerance,
    ``divergent`` when the mean ``lo`` exceeds it and ``uncertain``
    otherwise.  Rows with an interval are ranked from 1 by ascending
    mean ``hi`` with ties keeping the scenario order.

    The returned mapping uses the key order ``schema, tolerance, layers,
    data``; ``schema`` is ``climate-grid/cvm-v1``, ``tolerance`` echoes
    the shared tolerance and ``layers`` lists the ``element, region``
    pairs in item order.  ``data`` follows the scenario order; each row
    uses the key order ``scenario, count, states, interval, state,
    rank``.  ``count``, the ``states`` counts and ``rank`` are ints and
    every output float is ``round(x, 12)`` with negative zero normalized
    ...
    for index, item in enumerate(items):
        ...
        tolerance, factor, data = _validate_converge(
            item["result"], where=f"{where}.result"
        )
        ...
        if not validated:
            ...
            first_factor = factor
            first_scenarios = scenarios
            first_scenario_set = set(scenarios)
        else:
            if tolerance != first_tolerance or factor != first_factor:
                raise ValueError(
                    "all items must share the same result tolerance and "
                    "factor, taken from the first item"
                )
            if set(scenarios) != first_scenario_set:
                raise ValueError(
                    "all items must share the same result scenarios, "
                    "ordered as in the first item"
                )
    ...
    for s_index, scenario in enumerate(first_scenarios):
        collected = [
            lookup[scenario]
            for lookup in validated
            if lookup[scenario]["state"] is not None
            and lookup[scenario]["interval"] is not None
        ]
        n = len(collected)
        states = {
            state: sum(1 for row in collected if row["state"] == state)
            for state in _CONVERGE_STATES
        }
        if n < minimum:
            entries.append((s_index, scenario, n, states, None, None))
        else:
            lo = sum(row["interval"][0] for row in collected) / n
            hi = sum(row["interval"][1] for row in collected) / n
            entries.append((s_index, scenario, n, states, lo, hi))

    ranked = sorted(
        (entry for entry in entries if entry[4] is not None),
        key=lambda entry: (entry[5], entry[0]),
    )
    rank_of = {
        entry[0]: rank for rank, entry in enumerate(ranked, start=1)
    }

    rows = []
    for s_index, scenario, n, states, lo, hi in entries:
        if lo is None:
            interval = None
            state = None
            row_rank = None
            ...
            interval = [_round_output(lo), _round_output(hi)]
            if hi <= first_tolerance:
                state = "stable"
            elif lo > first_tolerance:
                state = "divergent"
            else:
                state = "uncertain"
            row_rank = rank_of[s_index]
        rows.append(
            {
                "scenario": scenario,
                "count": n,
...
_MATRIX_EVOLUTION_KEYS = ("schema", "periods", "models", "scenarios", "data")
...
def _validate_matrix_evolution(result: Any) -> tuple[list, list, list, list]:
    if not isinstance(result, dict):
        raise TypeError("result must be a dict")
    if tuple(result.keys()) != _MATRIX_EVOLUTION_KEYS:
        raise ValueError(
            "result must have exactly the keys schema, periods, models, "
            "scenarios, data in order"
        )
    ...
    if schema != _MATRIX_EVOLUTION_SCHEMA:
        raise ValueError(
            f"result.schema must be {_MATRIX_EVOLUTION_SCHEMA!r}"
        )

    periods = result["periods"]
    if not isinstance(periods, list):
        raise TypeError("result.periods must be a list")
    if len(periods) < 2:
        raise ValueError("result.periods must contain at least 2 periods")
    seen_periods: set[str] = set()
    for index, period in enumerate(periods):
        if not isinstance(period, str):
            raise TypeError(f"result.periods[{index}] must be a str")
        if period == "":
            raise ValueError(f"result.periods[{index}] must be non-empty")
        if period in seen_periods:
            raise ValueError(
                f"duplicate result.periods entry: {period!r}"
            )
        seen_periods.add(period)

    models = result["models"]
    if not isinstance(models, list):
        raise TypeError("result.models must be a list")
    if len(models) == 0:
        raise ValueError("result.models must be non-empty")
    seen_models: set[str] = set()
    for index, model in enumerate(models):
        if not isinstance(model, str):
            raise TypeError(f"result.models[{index}] must be a str")
        if model == "":
            raise ValueError(f"result.models[{index}] must be non-empty")
        if model in seen_models:
            raise ValueError(
                f"duplicate result.models entry: {model!r}"
            )
    ...
    for index, scenario in enumerate(scenarios):
        if not isinstance(scenario, str):
            raise TypeError(f"result.scenarios[{index}] must be a str")
        if scenario == "":
            raise ValueError(f"result.scenarios[{index}] must be non-empty")
        if scenario in seen_scenarios:
            raise ValueError(
                f"duplicate result.scenarios entry: {scenario!r}"
            )
        seen_scenarios.add(scenario)
    ...
    expected_rows = (len(periods) - 1) * len(models) * len(scenarios)
    if len(data) != expected_rows:
        raise ValueError(
            f"result.data must have {expected_rows} rows (one per adjacent "
            "period pair, model and scenario in period-pair-then-model-"
            "then-scenario order)"
        )

    state_values = set(_CONVERGE_MATRIX_STATE_KEYS)
    row_index = 0
    for left_index in range(len(periods) - 1):
        for model in models:
            for scenario in scenarios:
                ...
                if not isinstance(row["left"], str):
                    raise TypeError(f"{row_where}.left must be a str")
                if row["left"] != periods[left_index]:
                    raise ValueError(
                        f"{row_where}.left must be {periods[left_index]!r} "
                        "for its period-pair-then-model-then-scenario position"
                    )
                if not isinstance(row["right"], str):
                    raise TypeError(f"{row_where}.right must be a str")
                if row["right"] != periods[left_index + 1]:
                    raise ValueError(
                        f"{row_where}.right must be "
                        f"{periods[left_index + 1]!r} for its period-pair-"
                        "then-model-then-scenario position"
                    )
                ...
                if row["model"] != model:
                    raise ValueError(
                        ...
                        "period-pair-then-model-then-scenario position"
                    )
                if not isinstance(row["scenario"], str):
                    raise TypeError(f"{row_where}.scenario must be a str")
                if row["scenario"] != scenario:
                ...
                for end_name in ("from", "to"):
                    ...
                    if end is not None and end not in state_values:
                        raise ValueError(
                ...
                if interval is not None:
                    if not isinstance(interval, list):
                        raise TypeError(
                            f"{row_where}.interval must be a list or None"
                        )
                    if len(interval) != 2:
                        raise ValueError(
                            f"{row_where}.interval must have exactly 2 bounds"
                ...
                if ends_present:
                    if interval is None:
                        raise ValueError(
                            f"{row_where}.interval must be present when both "
                            "from and to are present"
                        )
                    if rank is None:
                        raise ValueError(
                            f"{row_where}.rank must be present when both "
                            "from and to are present"
                        )
                else:
                    if interval is not None:
...
def summarize_matrix_evolution(result, *, min_transitions: int = 1) -> dict:
    """Summarize :func:`matrix_evolution` transitions per model and scenario.

    ``result`` must be a complete :func:`matrix_evolution` result (schema
    ``climate-grid/cme-v1``) with exactly the keys ``schema, periods, models,
    scenarios, data`` in that order; every member is validated against that
    contract, including the flat period-pair-then-model-then-scenario
    ``data`` row order, each row's key order ``left, right, model, scenario,
    from, to, interval, rank`` and the ``from``/``to`` states (one of
    ``stable, uncertain, divergent`` or ``None``).  ``min_transitions``
    ...
    the number of the model/scenario's rows whose ``from`` and ``to`` are
    both not ``None``.  ``transitions`` is a list of nine int counts over
    the Cartesian product of the ``from`` and ``to`` states, ordered with
    ``from`` as the major key and ``to`` as the minor key, both in the
    order ``stable, uncertain, divergent``; rows with either end ``None``
    are not counted.

    When ``count`` is below ``min_transitions``, ``interval``,
    ``rank_mean`` and ``rank_abs`` are all ``None``.  Otherwise
    ``interval`` is ``[mean of those rows' first interval end, mean of
    their second interval end]``, ``rank_mean`` is the mean of their
    ``rank`` values and ``rank_abs`` is the mean of their absolute
    values; every counted row carries both an ``interval`` and a
    ``rank``.

    The returned mapping uses the key order ``schema, periods, models,
    scenarios, data``; ``schema`` is ``climate-grid/cme-summary-v1`` and
    the three axes echo the input arrays as-is.  ``data`` follows the
    model-then-scenario order; each row uses the key order ``model,
    ...
    for model in models:
        for scenario in scenarios:
            rows = grouped[(model, scenario)]
            transition_counts = [0] * 9
            interval_first: list[float] = []
            interval_second: list[float] = []
            ranks: list[int] = []
            for row in rows:
                if row["from"] is not None and row["to"] is not None:
                    pair_index = (
                        state_index[row["from"]] * 3
                        + state_index[row["to"]]
                    )
                    transition_counts[pair_index] += 1
                if row["interval"] is not None:
                    interval_first.append(row["interval"][0])
                    interval_second.append(row["interval"][1])
                if row["rank"] is not None:
                    ranks.append(row["rank"])

            count = sum(transition_counts)
            if count < min_transitions:
                ...
                interval = [
                    _round_output(
                        sum(interval_first) / len(interval_first)
                    ),
                    _round_output(
                        sum(interval_second) / len(interval_second)
                    ),
                ]
                rank_mean = _round_output(sum(ranks) / len(ranks))
                rank_abs = _round_output(
                    sum(abs(rank) for rank in ranks) / len(ranks)
                )
