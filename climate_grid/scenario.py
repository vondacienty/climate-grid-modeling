...
def _validate_rank_attribute_group_summary(
    ...
    _validate_string_list(summary["groups"], f"{where}.groups", minimum=2)
    scenarios = _validate_string_list(summary["scenarios"], f"{where}.scenarios")
...
def compare_attribute_layers(items) -> dict:
    """Compare layered rank-attribution group summaries against a baseline.

    ``items`` must be a list of at least two mappings, each with exactly the
    keys ``period, region, summary`` in that order.  ``period`` and
    ``region`` are non-empty strs and the ``(period, region)`` combination
    must be unique across the items.  ``summary`` is a complete
    :func:`summarize_rank_attribute_groups` result (schema
    ``climate-grid/ra-group-summary-v1``) with exactly the keys ``schema,
    groups, scenarios, elements, drivers, data`` in that order; every member
    is validated against that contract, including the flat
    scenario-then-element-then-driver ``data`` row order, each row's key
    order ``scenario, element, driver, group_count, count, contribution,
    uncertainty`` and the row invariants (``group_count`` and ``count``
    non-negative non-bool ints; ``contribution`` and ``uncertainty`` both
    ``None`` exactly when ``count`` is zero and both finite numbers
    otherwise, with ``uncertainty`` non-negative).  The items' ``groups``
    values may differ, but every item's summary must share the same
    ``scenarios``, ``elements`` and ``drivers`` in the same order; all three
    are taken from the first item.

    The first item is the baseline.  For every other item (a layer, in item
    order), scenario (in scenario order), element (in element order) and
    driver (in driver order), the layer row's ``contribution``/``uncertainty``
    are ``c``/``u`` and the baseline row's are ``c0``/``u0``; when all four
    are not ``None``, ``difference`` is ``c - c0`` and ``uncertainty`` is
    ``sqrt(u ** 2 + u0 ** 2)``, otherwise both are ``None``.

    The returned mapping uses the key order ``schema, reference, layers,
    scenarios, elements, drivers, data``; ``schema`` is
    ``climate-grid/ra-layer-v1``, ``reference`` is a dict of the first
    item's ``period`` and ``region``, ``layers`` lists the same-shaped
    dicts for the remaining items and ``scenarios``/``elements``/``drivers``
    echo the first item's summary axes.  ``data`` is a flat list of rows in
    layer-then-scenario-then-element-then-driver order; each row uses the
    key order ``scenario, element, driver, difference, uncertainty``.
    Every output float is ``round(x, 12)`` with negative zero normalized to
    ``0.0``.  Inputs are not modified.
    ...
    for index, item in enumerate(items):
        ...
        if tuple(item.keys()) != _RANK_ATTRIBUTE_LAYER_ITEM_KEYS:
            raise ValueError(
                f"{where} must have exactly the keys period, region, summary "
                "in order"
            )
        ...
        if pair in seen_pairs:
            raise ValueError(
                f"duplicate item period/region combination: {pair!r}"
            )
        seen_pairs.add(pair)
        ...
        if not validated_rows:
            ...
            if scenarios != first_scenarios:
                raise ValueError(
                    "all items must share the same summary scenarios in "
                    "the same order, taken from the first item"
                )
            if elements != first_elements:
                raise ValueError(
                    "all items must share the same summary elements in "
                    "the same order, taken from the first item"
                )
            if drivers != first_drivers:
                raise ValueError(
                    "all items must share the same summary drivers in "
                    "the same order, taken from the first item"
                )

        labels.append({"period": period, "region": region})
        validated_rows.append(rows)

    baseline_rows = validated_rows[0]
    result_data = []
    for rows in validated_rows[1:]:
        for scenario in first_scenarios:
            for element in first_elements:
                for driver in first_drivers:
                    key = (scenario, element, driver)
                    ...
                    contribution = row["contribution"]
                    uncertainty = row["uncertainty"]
                    base_contribution = baseline_row["contribution"]
                    base_uncertainty = baseline_row["uncertainty"]
                    if (
                        contribution is None
                        or uncertainty is None
                        or base_contribution is None
                        ...
                        combined_uncertainty = _round_output(
                            math.sqrt(
                                uncertainty ** 2 + base_uncertainty ** 2
                            )
                        )
                    result_data.append(
                        {
