...
def _weighted_autocorrelation_window_region(
    ...
    mean_x = (
        sum(w * x for w, x in zip(paired_weights, paired_x)) / weight_sum
    )
    mean_y = (
        sum(w * y for w, y in zip(paired_weights, paired_y)) / weight_sum
    )
    dx = [x - mean_x for x in paired_x]
    dy = [y - mean_y for y in paired_y]
    covariance = (
        sum(w * dxi * dyi for w, dxi, dyi in zip(paired_weights, dx, dy))
        / weight_sum
    )
    variance_x = sum(w * dxi ** 2 for w, dxi in zip(paired_weights, dx)) / weight_sum
    variance_y = sum(w * dyi ** 2 for w, dyi in zip(paired_weights, dy)) / weight_sum
    if variance_x == 0.0 or variance_y == 0.0:
        correlation = None
    else:
        correlation = covariance / math.sqrt(variance_x * variance_y)
...
def aggregate_weighted_autocorrelation(
    ...
    """Aggregate a grid element into weighted window lag-1 autocorrelation.

    Behaves like :func:`aggregate_autocorrelation` — ``temporal``,
    ``element``, ``regions``, ``windows`` and ``min_count`` follow the same
    validation, exceptions and adjacent-day pairing rules — but each
    region's cells carry explicit weights.  ``weights`` follows
    :func:`aggregate_weighted_window`: a non-empty list with one entry per
    region, in region order, each a dict with exactly the keys ``name`` and
    ``values`` in that order, where ``name`` is a non-empty str equal to the
    corresponding region's name and ``values`` holds one finite non-bool
    positive int or float per cell, aligned with that region's ``cells``.
    Wrong container or item types raise ``TypeError``; an empty list, an
    empty or mismatched name, or a non-finite or non-positive weight raises
    ``ValueError``.

    For every window (in window order) and region (in region order), every
    cell of every pair of adjacent days ``(t, t + 1)`` of the inclusive
    interval contributes a weighted sample
    ``(x, y, ux, uy, w) = (v[t], v[t + 1], u[t], u[t + 1], w)`` only when
    its status is non-``missing`` on both days; the window's last day and
    any missing day are skipped in pairing.  ``count`` is the number ``n``
    of such pairs; when ``n`` is below ``min_count``, ``correlation`` and
    ``uncertainty`` are both ``None``.  Otherwise, with ``W = sum(w)`` and
    the weighted centered differences ``dx = x - sum(w * x) / W`` and
    ``dy = y - sum(w * y) / W``, ``C = sum(w * dx * dy) / W``,
    ``Vx = sum(w * dx ** 2) / W`` and ``Vy = sum(w * dy ** 2) / W``;
    ``correlation`` is ``C / sqrt(Vx * Vy)``, or ``None`` when
    ``Vx * Vy`` is zero; and ``uncertainty`` is
    ``sqrt(sum((w * ux) ** 2 + (w * uy) ** 2)) / W`` over the paired
    samples.
