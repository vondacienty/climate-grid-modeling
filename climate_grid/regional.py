...
def aggregate_weighted_multi_variance(
    ...
    """Aggregate several elements into weighted per-window variance stats.

    Combines :func:`aggregate_multi_variance` and
    :func:`aggregate_weighted_variance`: ``temporal``, ``elements``,
    ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_weighted_variance` (wrong container/item/argument
    types raise ``TypeError``; every other contract violation raises
    ``ValueError``), except that ``elements`` is a non-empty list of
    unique non-empty str element names that must all exist in
    ``temporal.data`` (wrong item types raise ``TypeError``; an empty,
    duplicate or unknown element raises ``ValueError``).  ``weights``
    ...
    element (in element order), every non-``missing`` cell of every day
    of the inclusive interval contributes a weighted sample ``(v, u, w)``
    of value, uncertainty and cell weight.  ``count`` is the number of
    samples ``n`` and ``W = sum(w)`` over the samples; when ``n`` is below
    ``min_count``, ``mean``, ``variance``, ``stddev`` and ``uncertainty``
    are all ``None``.  Otherwise, with ``m = sum(w * v) / W``, the
    statistics are ``m``, ``sum(w * (v - m) ** 2) / W``,
    ...
    ``window, region, element, count, mean, variance, stddev,
    uncertainty``.  ``count`` is an int and every output float is
