...
def _correlation_window_region(
    ...
    for t in range(t_start, t_end + 1):
        for i, j in cells:
            if (
                status_x[t][i][j] != "missing"
                and status_y[t][i][j] != "missing"
            ):
                xs.append(values_x[t][i][j])
                ys.append(values_y[t][i][j])
                uxs.append(uncertainty_x[t][i][j])
                uys.append(uncertainty_y[t][i][j])
    ...
    mean_y = sum(ys) / count
    covariance = (
        sum((vx - mean_x) * (vy - mean_y) for vx, vy in zip(xs, ys)) / count
    )
    variance_x = sum((vx - mean_x) ** 2 for vx in xs) / count
    variance_y = sum((vy - mean_y) ** 2 for vy in ys) / count
    if variance_x == 0.0 or variance_y == 0.0:
        correlation = None
    else:
        correlation = covariance / math.sqrt(variance_x * variance_y)
    uncertainty = (
        math.sqrt(sum(ux ** 2 + uy ** 2 for ux, uy in zip(uxs, uys))) / count
    )
...
def aggregate_correlation(
    ...
    *,
    ...
    """Aggregate two reconstructed elements into per-window covariance stats.

    Behaves like :func:`aggregate_window` — ``temporal``, ``regions``,
    ``windows`` and ``min_count`` follow the same validation, exceptions,
    input order and closed-interval rules — but it pairs two elements,
    ``element_x`` and ``element_y``, each validated like ``element`` there:
    both must be non-empty str names that exist in ``temporal.data`` and they
    must differ from each other; a wrong name type raises ``TypeError`` while
    an empty, unknown or repeated name raises ``ValueError``.

    For every window (in window order) and region (in region order), a cell of
    a day contributes a paired sample ``(vx, vy, ux, uy)`` only when both
    elements are non-``missing`` at that cell.  ``count`` is the number ``n``
    of paired samples; when ``n`` is below ``min_count``, ``covariance``,
    ``correlation`` and ``uncertainty`` are all ``None``.  Otherwise, with
    ``mx = sum(vx) / n`` and ``my = sum(vy) / n``, the statistics are
    ``covariance = sum((vx - mx) * (vy - my)) / n``,
    ``correlation = covariance / sqrt(sx2 * sy2)`` where
    ``sx2 = sum((vx - mx) ** 2) / n`` and ``sy2`` is the analogous y
    variance (``None`` when either variance is zero), and
    ``uncertainty = sqrt(sum(ux ** 2 + uy ** 2)) / n`` over the paired
    samples' uncertainties.

    The returned mapping uses the key order ``schema, element_x, element_y,
    windows, regions, data``; ``schema`` is
    ``climate-grid/correlation-window-v1``, ``element_x``, ``element_y`` and
    ...
    order ``name, count, covariance, correlation, uncertainty``.  ``count``
    is an int and every output float is ``round(x, 12)`` with negative zero
    normalized to ``0.0``.  Inputs are not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)

    for label, element in (("element_x", element_x), ("element_y", element_y)):
        if not isinstance(element, str):
