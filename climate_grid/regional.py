...
def aggregate_weighted_change(
    ...
    follow the same validation, exceptions, input order and closed-interval
    rules — but instead of pooling samples it compares each cell's earliest
    and latest non-``missing`` observations inside the window, weighting each
    retained cell by its per-cell weight.
    ...
    change ``d = v1 - v0`` and a change uncertainty
    ``du = sqrt(u0 ** 2 + u1 ** 2)``.  ``count`` is the number of retained
    cells ``n``; when ``n`` is below ``min_count``, ``mean_change``,
    ``min_change``, ``max_change`` and ``uncertainty`` are all ``None``.
    Otherwise, with ``W = sum(w)`` over the retained cells, ``mean_change``
    is ``sum(w * d) / W``, ``min_change`` and ``max_change`` are the minimum
    and maximum cell changes (weights ignored) and ``uncertainty`` is
    ``sqrt(sum((w * du) ** 2)) / W`` over the retained cells.
