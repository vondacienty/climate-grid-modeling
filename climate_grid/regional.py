...
def _weighted_exceedance_window_region(
    ...
    return {
        ...
        "mean_excess": _round_output(excess_weight / weight_sum),
        "uncertainty": _round_output(
            math.sqrt(
                sum(
                    (w * u) ** 2
...
def aggregate_weighted_exceedance(
    ...
    """Aggregate a reconstructed grid series into weighted window exceedance stats.
    ...
    samples.  When ``n`` is below ``min_count``, everything except ``count``
    (``exceed_weight``, ``rate``, ``mean_excess`` and ``uncertainty``) is
    ``None``.  Otherwise ``exceed_weight`` is ``sum(w for v > threshold)``,
    ``rate`` is ``exceed_weight / W``, ``mean_excess`` is
    ...
    ``data`` follows the window order; each entry uses the key order
    ``name, regions``, where each region entry uses the key order
    ``name, count, exceed_weight, rate, mean_excess, uncertainty``.  ``count``
