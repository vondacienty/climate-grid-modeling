...
def _regression_window_region(
    ...
    return {
        ...
        "intercept": None if intercept is None else _round_output(intercept),
        "uncertainty": _round_output(uncertainty),
    }
...
def aggregate_regression(
    ...
    Behaves like :func:`aggregate_window` — ``temporal``, ``regions``,
    ``windows`` and ``min_count`` follow the same validation, exceptions and
    closed-interval sampling rules — but it fits a linear regression between
    two elements instead of summarizing one.  ``element_x`` and ``element_y``
    must each be a non-empty str naming an element of ``temporal.data`` and
    they must name different elements; wrong element types raise
    ...
    is non-``missing`` for *both* elements.  ``count`` is the number of paired
    samples ``n``; when ``n`` is below ``min_count``, ``slope``,
    ``intercept`` and ``uncertainty`` are all ``None``.  Otherwise
    ``x_bar = sum(x) / n`` and ``y_bar = sum(y) / n``;
    ``sxx = sum((x - x_bar) ** 2)`` and
    ``sxy = sum((x - x_bar) * (y - y_bar))``; when ``sxx`` is zero,
    ``slope`` and ``intercept`` are ``None``, otherwise ``slope = sxy / sxx``
    and ``intercept = y_bar - slope * x_bar``; and ``uncertainty`` is
    ``sqrt(sum(ux ** 2 + uy ** 2)) / n`` over the paired samples.
