...
def aggregate_weighted_autocorrelation(
    ...
    """Aggregate a reconstructed grid element into weighted per-window lag-1 autocorrelation.

    Behaves like :func:`aggregate_autocorrelation` — ``temporal``,
    ``element``, ``regions``, ``windows`` and ``min_count`` follow the same
    validation, exceptions and adjacent-day pairing rules — but each region's
    cells carry explicit weights, as in
    :func:`aggregate_weighted_correlation`.  ``weights`` is a non-empty list
    with one entry per region, in region order, each a dict with exactly the
    keys ``name`` and ``values`` in that order, where ``name`` equals the
    corresponding region's name and ``values`` holds one finite non-bool
    positive int or float per cell, aligned with that region's ``cells``.
    ...
    interval contributes a paired weighted sample
    ``(x, y, ux, uy, w) = (v[t], v[t + 1], u[t], u[t + 1], w)`` only when its
    status is non-``missing`` on both days; the last day of the window and
    ``missing`` days never start or complete a pair.  ``count`` is the number
    ``n`` of such pairs; when ``n`` is below ``min_count``, ``correlation``
    and ``uncertainty`` are both ``None``.  Otherwise, with ``W = sum(w)``
    and centered differences ``dx = x - sum(w * x) / W`` and
    ``dy = y - sum(w * y) / W``, ``C = sum(w * dx * dy) / W``,
    ``Vx = sum(w * dx ** 2) / W`` and ``Vy = sum(w * dy ** 2) / W``;
    ``correlation`` is ``C / sqrt(Vx * Vy)``, or ``None`` when ``Vx * Vy`` is
    zero; and ``uncertainty`` is
    ``sqrt(sum((w * ux) ** 2 + (w * uy) ** 2)) / W`` over the paired samples.
