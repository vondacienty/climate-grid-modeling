    ...
    for q in quantiles:
        ...
        for v, w, _u in ordered:
            ...
            if cumulative >= target:
                selected = v
                break
        quantile_values.append(_round_output(selected))
...
def aggregate_weighted_quantile(
    ...
    non-``missing`` sampling rules — but instead of mean/min/max it reports
    weighted quantiles.  ``quantiles`` is a non-empty list of finite non-bool
    numbers with ``0 <= q <= 1`` in strictly increasing order; wrong item
    types raise ``TypeError`` while an empty, out-of-range or non-increasing
    list raises ``ValueError``.

    For every window (in window order) and region (in region order), every
    non-``missing`` cell of every day of the inclusive interval contributes a
    weighted sample ``(v, u, w)``, where ``w`` is that cell's weight.
    ...
    sorted by ``(v, w)`` ascending and each quantile ``q`` is the value ``v``
    of the first sample whose cumulative weight reaches ``q * W``, where
    ``W`` is the sum of the sample weights; ``q = 0`` therefore yields the
    minimum sample value.  ``uncertainty`` is ``sqrt(sum((w * u) ** 2)) / W``
    over the samples' uncertainties.
    ...
    argument, ``quantiles`` and ``windows`` list three-key dicts in input
    order and ``regions`` lists the region names in input order.  ``data``
    follows the window order; each entry uses the key order ``name,
    regions``, where each region entry uses the key order ``name, count,
    quantiles, uncertainty``.  ``count`` is an int and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  Inputs are
    not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
