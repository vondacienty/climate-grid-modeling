...
def aggregate_weighted_variance(
    ...
    """Aggregate a reconstructed grid series into weighted window variance.
    ...
    weighted sample ``(v, u, w)`` of value, uncertainty and cell weight.
    ``count`` is the number of samples; when it is below ``min_count``,
    ``mean``, ``variance``, ``stddev`` and ``uncertainty`` are all ``None``.
    Otherwise, with ``W = sum(w)`` and ``m = sum(w * v) / W`` over the
    samples, they are ``m``, ``sum(w * (v - m) ** 2) / W``,
    ``sqrt(sum(w * (v - m) ** 2) / W)`` and
    ``sqrt(sum((w * u) ** 2)) / W`` over the samples' uncertainties.

    The returned mapping uses the key order ``schema, element, windows,
    regions, data``; ``schema`` is
    ``climate-grid/weighted-variance-window-v1``, ``element`` echoes the
    argument, ``windows`` lists three-key dicts in input order and ``regions``
    ...
    uses the key order ``name, count, mean, variance, stddev, uncertainty``.
