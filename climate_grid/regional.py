...
def aggregate_weighted_regression(
    ...
    Behaves like :func:`aggregate_weighted_correlation` — ``temporal``,
    ``regions``, ``windows``, ``weights`` and ``min_count`` follow the same
    validation, exceptions and closed-interval paired weighted sampling
    rules, and ``element_x`` and ``element_y`` must each be a non-empty str
    naming an element of ``temporal.data`` and name different elements
    (wrong element types raise ``TypeError`` while an empty, unknown or
    repeated element raises ``ValueError``) — but it fits a weighted
    least-squares line instead of computing correlation.
    ...
    ``Sxy = sum(w * (x - x_bar) * (y - y_bar))``, ``slope`` and
    ``intercept`` are ``None`` when ``Sxx`` is zero and otherwise
    ``slope = Sxy / Sxx`` and ``intercept = y_bar - slope * x_bar``;
    ``uncertainty`` is ``sqrt(sum((w * ux) ** 2 + (w * uy) ** 2)) / W``
    over the paired samples.
