...
def aggregate_weighted_multi_autocorr(
    ...
    """Aggregate several elements into weighted window lag-1 autocorrelation at once.

    Combines :func:`aggregate_multi_autocorr` and
    :func:`aggregate_weighted_autocorrelation`: ``temporal``, ``regions``,
    ``windows`` and ``min_count`` follow :func:`aggregate_window`;
    ``elements`` is a non-empty list of unique non-empty str element names
    that must all exist in ``temporal.data`` (wrong item types raise
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``); and ``weights`` follows
    ...
    cell's weight.  ``count`` is the number ``n`` of such pairs; when ``n``
    is below ``min_count``, ``correlation`` and ``uncertainty`` are both
    ``None``.  Otherwise, with ``W = sum(w)``, ``x_bar = sum(w * x) / W``
    and ``y_bar = sum(w * y) / W``; ``C = sum(w * (x - x_bar) * (y - y_bar))
    / W``, ``Vx = sum(w * (x - x_bar) ** 2) / W`` and ``Vy = sum(w * (y -
    y_bar) ** 2) / W``; ``correlation`` is ``C / sqrt(Vx * Vy)``, or
    ``None`` when either variance is zero; and ``uncertainty`` is
