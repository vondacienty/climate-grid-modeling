...
def aggregate_entropy(
    ...
    ``regions``, ``windows`` and ``min_count`` follow the same validation,
    exceptions and sampling rules — but instead of mean/min/max it reports
    the Shannon entropy of the sample value distribution.  ``edges`` is a
    ...
    For every window (in window order) and region (in region order), every
    non-``missing`` cell of every day of the inclusive interval contributes a
    sample ``(v, u)`` of value and uncertainty.  Samples are binned exactly
    as in :func:`aggregate_histogram` into ``B = len(edges) + 1`` bins.
    ``count`` is the number of samples ``n`` and ``bin_counts`` lists the
    ``B`` int bin counts.  When ``n`` is below ``min_count``, ``entropy``
    and ``uncertainty`` are both ``None``.  Otherwise, with
    ``p[i] = bin_counts[i] / n``, ``entropy`` is
    ``-sum(p[i] * log2(p[i]))`` over the bins with ``p[i] > 0`` and
