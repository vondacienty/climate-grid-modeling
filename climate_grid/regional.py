...
def aggregate_weighted_multi_window(
    ...
    """Aggregate several elements into weighted per-window region stats at once.

    Combines :func:`aggregate_multi` and :func:`aggregate_weighted_window`:
    ``temporal``, ``regions`` and ``min_count`` follow :func:`aggregate`;
    ``elements`` is a non-empty list of unique non-empty str element names
    that must all exist in ``temporal.data`` (wrong item types raise
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``); ``windows`` and ``weights`` follow
    :func:`aggregate_weighted_window` — ``windows`` a non-empty list of
    dicts with exactly the keys ``name``, ``start`` and ``end`` in that
    order, where ``name`` is a unique non-empty str and ``start``/``end``
    are valid ``YYYY-MM-DD`` dates occurring in ``temporal.times`` with
    ...
    minimum and maximum sample values (weights ignored) and
    ``uncertainty`` is ``sqrt(sum((w * u) ** 2)) / sum(w)`` over the
    samples' uncertainties.

    The returned mapping uses the key order ``schema, elements, windows,
    regions, data``; ``schema`` is
    ``climate-grid/weighted-multi-window-v1``, ``elements`` and ``windows``
