...
def _coverage_window_region(
    ...
    total = n_days * len(cells)
    available = 0
    observed = 0
    interpolated = 0
    sum_squared_uncertainty = 0.0
    for t in range(t_start, t_end + 1):
        for i, j in cells:
            ...
            if cell_status == "observed":
                ...
                interpolated += 1
            sum_squared_uncertainty += uncertainty[t][i][j] ** 2
    ...
    return {
        ...
        "uncertainty": _round_output(
            math.sqrt(sum_squared_uncertainty) / available
        ),
    }
...
def aggregate_coverage(
    ...
    Behaves like :func:`aggregate_window` — ``temporal``, ``element``,
    ``regions``, ``windows`` and ``min_count`` follow the same validation,
    exceptions and window/region ordering — but instead of value statistics
    it reports coverage of the grid over each window.
    ...
    number of region cells.  ``available`` is the number of those
    cell-days whose status is not ``missing``; ``observed`` and
    ``interpolated`` count the two non-``missing`` statuses separately, so
    ``available == observed + interpolated``.  When ``available`` is below
    ``min_count``, ``rate``, ``observed_rate``, ``interpolated_rate`` and
    ``uncertainty`` are all ``None``.  Otherwise they are
    ``available / total``, ``observed / total``, ``interpolated / total``
    and ``sqrt(sum(u ** 2)) / available`` over the available cell-days'
    uncertainties.
    ...
    ``name, total, available, observed, interpolated, rate,
    observed_rate, interpolated_rate, uncertainty``.  ``total``,
    ``available``, ``observed`` and ``interpolated`` are ints and every
    output float is ``round(x, 12)`` with negative zero normalized to
    ``0.0``.  Inputs are not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
