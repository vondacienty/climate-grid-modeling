...
def aggregate_multi_variance(
    ...
    Combines :func:`aggregate_multi_window` and :func:`aggregate_variance`:
    ``temporal`` and ``regions`` follow :func:`aggregate`; ``elements`` is a
    non-empty list of unique non-empty str element names that must all exist
    in ``temporal.data`` (wrong item types raise ``TypeError``; an empty,
    duplicate or unknown element raises ``ValueError``); ``windows`` and
    ``min_count`` follow :func:`aggregate_variance` — wrong
    container/item/argument types raise ``TypeError`` and every other
