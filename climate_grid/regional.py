...
def aggregate_weighted_autocorrelation(
    ...
    its status is non-``missing`` on both days; a missing day breaks pairing
    across it.  ``count`` is the number ``n`` of such pairs; when ``n`` is
    below ``min_count``, ``correlation`` and ``uncertainty`` are both
    ``None``.  Otherwise, with ``W = sum(w)``, ``x_bar = sum(w * x) / W``
    and ``y_bar = sum(w * y) / W``; ``d = (x - x_bar, y - y_bar)``;
    ``C = sum(w * dx * dy) / W``, ``Vx = sum(w * dx ** 2) / W`` and
    ``Vy = sum(w * dy ** 2) / W``; ``correlation`` is
    ``C / sqrt(Vx * Vy)``, or ``None`` when ``Vx * Vy`` is zero; and
    ``uncertainty`` is
