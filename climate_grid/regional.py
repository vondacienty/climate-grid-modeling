...
def aggregate_multi_coverage(
    ...
    that must all exist in ``temporal.data`` (wrong item types raise
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``); ``windows`` follows :func:`aggregate_coverage` — a
    non-empty list of dicts with exactly the keys ``name``, ``start`` and
    ``end`` in that order, where ``name`` is a unique non-empty str and
    ``start``/``end`` are valid ``YYYY-MM-DD`` dates occurring in
    ``temporal.times`` with ``start <= end``.
    ...
    For every window (in window order), region (in region order) and element
    (in element order), ``total`` is the number of days in the inclusive
    interval times the number of cells in the region.  ``available`` counts
    non-``missing`` cells, split into ``observed`` and ``interpolated``; all
    four counts are ints.  When ``available`` is below ``min_count``,
    ``rate``, ``observed_rate``, ``interpolated_rate`` and ``uncertainty``
    are all ``None``.  Otherwise they are ``available / total``,
    ``observed / total``, ``interpolated / total`` and
    ``sqrt(sum(u ** 2)) / available`` over the available cells'
    uncertainties.

    The returned mapping uses the key order ``schema, elements, windows,
    regions, data``; ``schema`` is
    ``climate-grid/multi-coverage-window-v1``, ``elements`` and ``windows``
    are passed through unchanged and ``regions`` lists the region names in
    input order.  ``data`` is a flat list of rows in
    ...
    rate, observed_rate, interpolated_rate, uncertainty``.  Every output
