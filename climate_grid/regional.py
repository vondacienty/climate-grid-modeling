...
def aggregate_weighted_multi_entropy(
    ...
    """Aggregate several elements into weighted per-window value entropies.
    ...
    are binned exactly as in :func:`aggregate_weighted_histogram`,
    ``count`` is the number of samples ``n`` and ``bin_weights[i]`` is the
    sum of the weights of the samples counted in bin ``i``; all ``B`` entries
    are always present.  When ``n`` is below ``min_count``, ``entropy`` and
    ``uncertainty`` are both ``None``.  Otherwise, with
    ...
    The returned mapping uses the key order ``schema, elements, edges,
    windows, regions, data``; ``schema`` is
    ``climate-grid/weighted-multi-entropy-window-v1``, ``elements``,
    ``edges`` and ``windows`` are passed through unchanged and ``regions``
    lists the region names in input order.  ``data`` is a flat list of rows in
    window-then-region-then-element order; each row uses the key order
