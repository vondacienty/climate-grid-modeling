...
def aggregate_exceedance(
    ...
    ``regions``, ``windows`` and ``min_count`` follow the same validation,
    exceptions and sampling rules — but instead of mean/min/max it reports
    exceedance statistics against ``threshold``.  ``threshold`` must be a
    finite non-bool number; a wrong type raises ``TypeError`` and a
    non-finite value (``NaN``/infinity) raises ``ValueError``.

    For every window (in window order) and region (in region order), every
    non-``missing`` cell of every day of the inclusive interval contributes a
    sample ``(v, u)`` of value and uncertainty.  ``count`` is the number of
    samples; when it is below ``min_count``, every statistic other than
    ``count`` is ``None``.  Otherwise ``exceed`` is ``sum(v > threshold)``,
    ...
    windows, regions, data``; ``schema`` is
    ``climate-grid/exceedance-window-v1``, ``element`` echoes the argument,
    ``threshold`` and ``windows`` are echoed unchanged and ``regions`` lists
    ...
    the key order ``name, count, exceed, rate, mean_excess, uncertainty``.
    ``count`` and ``exceed`` are ints and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  Inputs are
    not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
