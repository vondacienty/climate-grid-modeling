...
__all__ = [
    ...
    "aggregate_weighted_multi_entropy",
]
...
_MULTI_REGRESSION_SCHEMA = "climate-grid/mreg-v1"
...
_WEIGHTED_MULTI_ENTROPY_SCHEMA = "climate-grid/weighted-multi-entropy-window-v1"
...
def _skewness_window_region(
    ...
    if variance == 0.0:
        ...
        skewness = sum(((v - mean) / stddev) ** 3 for v in window_values) / count
    return {
        "count": count,
        "mean": _round_output(mean),
        "variance": _round_output(variance),
...
def _multi_skew_window_region(
    ...
    if variance == 0.0:
        ...
        skewness = (
            sum((v - mean) ** 3 for v in window_values)
            / (count * variance ** 1.5)
        )
    return {
        "count": count,
...
def _weighted_autocorrelation_window_region(
    ...
    variance_x = (
        sum(
            w * (x - mean_x) ** 2
            for w, x in zip(paired_weights, paired_x)
        )
        / weight_sum
    )
    variance_y = (
        sum(
            w * (y - mean_y) ** 2
            for w, y in zip(paired_weights, paired_y)
        )
        / weight_sum
    )
    if variance_x == 0.0 or variance_y == 0.0:
...
def _entropy_window_region(
    ...
    entropy = -sum(
        (bin_count / count) * math.log2(bin_count / count)
...
def aggregate_skewness(
    ...
    ``regions``, ``windows`` and ``min_count`` follow the same validation,
    exceptions, input order, closed-interval and non-``missing`` sampling
    rules — but instead of mean/min/max it reports moment statistics.

    For every window (in window order) and region (in region order), every
    non-``missing`` cell of every day of the inclusive interval contributes a
    sample ``(v, u)`` of value and uncertainty.  ``count`` is the number of
    samples ``n``; when ``n`` is below ``min_count``, ``mean``, ``variance``,
    ``skewness`` and ``uncertainty`` are all ``None``.  Otherwise, with
    ``m = sum(v) / n`` and ``variance = sum((v - m) ** 2) / n``, ``mean`` is
    ``m``, ``variance`` is that variance, ``skewness`` is ``None`` when the
    variance is zero and ``sum(((v - m) / sqrt(variance)) ** 3) / n``
    otherwise, and ``uncertainty`` is ``sqrt(sum(u ** 2)) / n`` over the
    samples' uncertainties.
...
def aggregate_kurtosis(
    ...
    """Aggregate a reconstructed grid series into per-window kurtosis stats.
    ...
    sample ``(v, u)`` of value and uncertainty.  ``count`` is the number of
    samples ``n``; when ``n`` is below ``min_count``, ``mean``, ``variance``,
    ``kurtosis`` and ``uncertainty`` are all ``None``.  Otherwise, with
    ``m = sum(v) / n`` and ``variance = sum((v - m) ** 2) / n``, ``mean`` is
    ``m``, ``variance`` is that variance, ``kurtosis`` is ``None`` when the
    variance is zero and ``sum(((v - m) / sqrt(variance)) ** 4) / n - 3``
    otherwise (excess kurtosis), and ``uncertainty`` is
    ``sqrt(sum(u ** 2)) / n`` over the samples' uncertainties.

    The returned mapping uses the key order ``schema, element, windows,
    regions, data``; ``schema`` is ``climate-grid/kurtosis-window-v1``,
    ``element`` and ``windows`` echo the arguments unchanged and ``regions``
    lists the region names in input order.  ``data`` follows the window
    order; each entry uses the key order ``name, regions``, where each region
    entry uses the key order ``name, count, mean, variance, kurtosis,
    uncertainty``.  ``count`` is an int and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  Inputs are
    not modified.
    """
...
def aggregate_multi_skew(
    temporal, elements, regions, windows, *, min_count: int = 1
) -> dict:
    """Aggregate several elements into per-window skewness stats at once.
    ...
    inclusive interval contributes a sample ``(v, u)`` of value and
    uncertainty.  ``count`` is the number of samples ``n``; when ``n`` is
    below ``min_count``, ``mean``, ``variance``, ``skewness`` and
    ``uncertainty`` are all ``None``.  Otherwise, with ``m = sum(v) / n`` and
    ``variance = sum((v - m) ** 2) / n``, ``mean`` is ``m``, ``variance`` is
    that variance, ``skewness`` is ``None`` when the variance is zero and
    ``sum((v - m) ** 3) / (n * variance ** 1.5)`` otherwise, and
    ``uncertainty`` is ``sqrt(sum(u ** 2)) / n`` over the samples'
    uncertainties.

    The returned mapping uses the key order ``schema, elements, windows,
    regions, data``; ``schema`` is ``climate-grid/multi-skew-v1``,
    ``elements`` and ``windows`` are passed through unchanged and ``regions``
    lists the region names in input order.  ``data`` is a flat list of rows
    in window-then-region-then-element order; each row uses the key order
    ``window, region, element, count, mean, variance, skewness,
    uncertainty``.  ``count`` is an int and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  Inputs are
    not modified.
    """
    ...
    for w_name, _start, _end, t_start, t_end in validated_windows:
        for r_name, cells in validated_regions:
            for element in elements:
                series = data[element]
                stats = _multi_skew_window_region(
                    series["values"],
                    series["status"],
                    series["uncertainty"],
                    cells,
...
def aggregate_multi_coverage(
    temporal, elements, regions, windows, *, min_count: int = 1
) -> dict:
    """Aggregate several elements into per-window coverage stats at once.

    Combines :func:`aggregate_multi` and :func:`aggregate_coverage`:
    ``temporal``, ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_coverage`, and ``elements`` follows
    ...
    element (in element order), ``total`` is the number of days in the
    inclusive interval times the number of cells in the region.
    ``available`` counts non-``missing`` cells, split into ``observed`` and
    ...
    Otherwise they are ``available / total``, ``observed / total``,
    ``interpolated / total`` and ``sqrt(sum(u ** 2)) / available`` over
    the available cells' uncertainties.

    The returned mapping uses the key order ``schema, elements, windows,
    regions, data``; ``schema`` is ``climate-grid/multi-coverage-window-v1``,
    ``elements`` and ``windows`` are passed through unchanged and ``regions``
    lists the region names in input order.  ``data`` is a flat list of rows
    in window-then-region-then-element order; each row uses the key order
...
def aggregate_multi_exceedance(
    ...
    uncertainty.  ``count`` is the number of samples ``n``; when
    ``n < min_count``, ``exceed``, ``rate``, ``mean_excess`` and
    ``uncertainty`` are all ``None``.  Otherwise ``exceed`` is the count
    of samples with ``v > threshold``, ``rate`` is ``exceed / n``,
    ...
    windows, regions, data``; ``schema`` is
    ``climate-grid/multi-exceedance-window-v1``, ``elements``,
    ``threshold`` and ``windows`` are passed through unchanged and
...
def aggregate_entropy(
    ...
    """Aggregate a reconstructed grid series into per-window value entropy.

    Behaves like :func:`aggregate_window` — ``temporal``, ``element``,
    ``regions``, ``windows`` and ``min_count`` follow the same validation,
    exceptions and sampling rules — but instead of mean/min/max it reports the
    Shannon entropy (base 2) of a histogram of sample values.  ``edges`` is a
    ...
    non-``missing`` cell of every day of the inclusive interval contributes a
    sample ``(v, u)`` of value and uncertainty.  With ``B = len(edges) + 1``
    bins, samples are binned exactly as in :func:`aggregate_histogram`,
    ``count`` is the number of samples ``n`` and ``bin_counts`` lists the
    ``B`` int bin counts.  When ``n`` is below ``min_count``, ``entropy`` and
    ``uncertainty`` are both ``None``.  Otherwise, with
    ``p = bin_counts[i] / n``, ``entropy`` is ``-sum(p * log2(p))`` over the
    bins with ``p > 0`` and ``uncertainty`` is ``sqrt(sum(u ** 2)) / n`` over
    the samples' uncertainties.

    The returned mapping uses the key order ``schema, element, edges,
    windows, regions, data``; ``schema`` is
    ``climate-grid/entropy-window-v1``, ``element`` echoes the argument,
    ``edges`` and ``windows`` are echoed in input order and ``regions`` lists
    the region names in input order.  ``data`` follows the window order; each
    entry uses the key order ``name, regions``, where each region entry uses
    the key order ``name, count, bin_counts, entropy, uncertainty``.
...
def aggregate_weighted_entropy(
    ...
    of weighted rates it reports the Shannon entropy (base 2) of a weighted
    ...
    Otherwise, with ``W = sum(bin_weights)`` and
    ``p_i = bin_weights[i] / W``, ``entropy`` is
    ``-sum(p_i * log2(p_i))`` over the bins with ``p_i > 0`` and
    ``uncertainty`` is ``sqrt(sum((w * u) ** 2)) / W`` over the samples'
    uncertainties.
    ...
    uncertainty``.  ``count`` is an int, ``bin_weights`` holds ``B`` floats
    and every output float is ``round(x, 12)`` with negative zero normalized
    to ``0.0``.  Inputs are not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
...
def aggregate_multi_trend(
    ...
    """Aggregate several elements into per-window region trend statistics.

    Combines :func:`aggregate_multi_window` and :func:`aggregate_trend`:
    ``temporal``, ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_window` — the same validation, exceptions, input
    order, closed-interval and non-``missing`` sampling rules — and
    ``elements`` is a non-empty list of unique non-empty str element names
    that must all exist in ``temporal.data`` (wrong item types raise
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``).
    ...
    For every window (in window order), region (in region order) and element
    (in element order), each day of the inclusive interval whose number
    ``n`` of non-``missing`` cells satisfies ``n >= min_count`` is retained,
...
def aggregate_multi_correlation(
    ...
    """Aggregate element pairs into per-window correlation stats at once.

    Combines :func:`aggregate_multi` and :func:`aggregate_correlation`:
    ``temporal``, ``regions`` and ``min_count`` follow :func:`aggregate`;
    ``elements`` is a non-empty list of unique non-empty str element names
    ...
    order) is treated like :func:`aggregate_correlation`: for every window
    (in window order) and region (in region order), a cell of a day
    contributes a paired sample ``(vx, vy, ux, uy)`` only when its status is
    non-``missing`` for both elements.  ``count`` is the number ``n`` of
    paired samples; when ``n`` is below ``min_count``, ``covariance``,
    ``correlation`` and ``uncertainty`` are all ``None``.  Otherwise
    ``dx`` and ``dy`` are the de-meaned samples; ``covariance`` is
    ``sum(dx * dy) / n``; the variances are ``sum(dx ** 2) / n`` and
    ``sum(dy ** 2) / n``; ``correlation`` is
    ...
    The returned mapping uses the key order ``schema, elements, windows,
    regions, data``; ``schema`` is
    ``climate-grid/multi-correlation-window-v1``, ``elements`` and
...
def aggregate_weighted_autocorrelation(
    ...
    """Aggregate a reconstructed grid element into weighted window lag-1 autocorrelation.

    Behaves like :func:`aggregate_autocorrelation` — ``temporal``,
    ``element``, ``regions``, ``windows`` and ``min_count`` follow the same
    validation, exceptions, closed-interval and adjacent-day paired sampling
    rules — but each region's cells carry explicit weights.  ``weights``
    follows :func:`aggregate_weighted_window`: a non-empty list with one
    entry per region, in region order, each a dict with exactly the keys
    ``name`` and ``values`` in that order, where ``name`` equals the
    ...
    positive int or float per cell, aligned with that region's ``cells``.
    Wrong container, entry, ``name`` or weight types raise ``TypeError``;
    every other weights contract violation raises ``ValueError``.

    For every window (in window order) and region (in region order), every
    cell of every pair of adjacent days ``(t, t + 1)`` of the inclusive
    interval contributes a paired weighted sample ``(x, y, ux, uy, w) =
    (v[t], v[t + 1], u[t], u[t + 1], w)`` only when its status is
    non-``missing`` on both days, where ``w`` is that cell's weight.
    ...
    W``, ``Vx = sum(w * (x - x_bar) ** 2) / W`` and ``Vy = sum(w * (y -
    y_bar) ** 2) / W``; ``correlation`` is ``C / sqrt(Vx * Vy)``, or ``None``
    when ``Vx * Vy`` is zero; and ``uncertainty`` is ``sqrt(sum((w * ux) **
    2 + (w * uy) ** 2)) / W`` over the paired samples.
    ...
    lists the region names in input order.  ``data`` follows the window
    order; each entry uses the key order ``name, regions``, where each
    region entry uses the key order ``name, count, correlation,
    uncertainty``.  ``count`` is an int and every output float is
    ``round(x, 12)`` with negative zero normalized to ``0.0``.  Inputs are
    not modified.
    """
...
def aggregate_multi_regression(
    ...
    return {
        "schema": _MULTI_REGRESSION_SCHEMA,
        "elements": elements,
        "windows": windows,
        "regions": [name for name, _ in validated_regions],
        "data": result_data,
...
def aggregate_multi_autocorr(
    ...
    ``windows`` and ``min_count`` follow :func:`aggregate_window`, and
    ``elements`` is a non-empty list of unique non-empty str element names
    that must all exist in ``temporal.data`` (wrong item types raise
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``).
...
def aggregate_weighted_multi_autocorr(
    ...
    :func:`aggregate_weighted_autocorrelation`: ``temporal``, ``elements``,
    ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_window`-style validation — ``elements`` is a non-empty
    list of unique non-empty str element names that must all exist in
    ``temporal.data`` (wrong item/argument types raise ``TypeError``; an
    empty, duplicate or unknown element raises ``ValueError``) — and
    ``weights`` follows :func:`aggregate_weighted_window`: a non-empty list
    with one entry per region, in region order, each a dict with exactly the
    keys ``name`` and ``values`` in that order, where ``name`` equals the
    corresponding region's name and ``values`` holds one finite non-bool
    ...
    cell's weight; the last day and any missing day are skipped.  ``count``
    is the number ``n`` of such pairs; when ``n`` is below ``min_count``,
    ``correlation`` and ``uncertainty`` are both ``None``.  Otherwise the
    statistics follow :func:`aggregate_weighted_autocorrelation`: with
    ``W = sum(w)``, ``x_bar = sum(w * x) / W`` and ``y_bar = sum(w * y) /
    W``; ``C = sum(w * (x - x_bar) * (y - y_bar)) / W``,
    ``Vx = sum(w * (x - x_bar) ** 2) / W`` and
    ``Vy = sum(w * (y - y_bar) ** 2) / W``; ``correlation`` is
    ``C / sqrt(Vx * Vy)``, or ``None`` when ``Vx * Vy`` is zero; and
    ``uncertainty`` is ``sqrt(sum((w * ux) ** 2 + (w * uy) ** 2)) / W`` over
    the paired samples.
...
def aggregate_multi_quantile(
    temporal, elements, regions, windows, quantiles, *, min_count: int = 1
) -> dict:
    """Aggregate several elements into per-window region quantiles at once.

    Combines :func:`aggregate_multi` and :func:`aggregate_quantile`:
    ``temporal``, ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_window`, ``elements`` is a non-empty list of unique
    ...
    element raises ``ValueError``), and ``quantiles`` is a non-empty list of
    finite non-bool numbers with ``0 <= q <= 1`` in strictly increasing
    order (wrong item types raise ``TypeError``; an empty, out-of-range or
    non-increasing list raises ``ValueError``).

    For every window (in window order), region (in region order) and element
    (in element order), every non-``missing`` cell of every day of the
    inclusive interval contributes a sample.  ``count`` is the number ``n``
    of samples; when ``n`` is below ``min_count``, ``quantiles`` is a list
    ...
    ``b = ceil(h)``, ``v[a] + (h - a) * (v[b] - v[a])``; ``uncertainty`` is
    ``sqrt(sum(u ** 2)) / n`` over the samples' uncertainties.

    The returned mapping uses the key order ``schema, elements, quantiles,
    windows, regions, data``; ``schema`` is
    ``climate-grid/multi-quantile-window-v1``, ``elements``, ``quantiles``
    and ``windows`` are passed through unchanged and ``regions`` lists the
    region names in input order.  ``data`` is a flat list of rows in
    window-then-region-then-element order; each row uses the key order
    ``window, region, element, count, quantiles, uncertainty``.  ``count``
    is an int and every output float is ``round(x, 12)`` with negative zero
    normalized to ``0.0``.  Inputs are not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
...
def aggregate_weighted_multi_quantile(
    ...
    """Aggregate several elements into weighted per-window region quantiles.
    ...
    raise ``TypeError``; every other contract violation raises
    ``ValueError``), ``weights`` follows
    :func:`aggregate_weighted_window` — a non-empty list with one entry per
    region, in region order, each a dict with exactly the keys ``name`` and
    ``values`` in that order, where ``name`` equals the corresponding
    region's name and ``values`` holds one finite non-bool positive int or
    float per cell, aligned with that region's ``cells`` — and
    ``quantiles`` is a non-empty list of finite non-bool numbers with
    ``0 <= q <= 1`` in strictly increasing order.
...
def aggregate_multi_entropy(
    ...
    """Aggregate several elements into per-window value entropies at once.

    Combines :func:`aggregate_multi` and :func:`aggregate_entropy`:
    ``temporal``, ``regions``, ``windows`` and ``min_count`` follow
    :func:`aggregate_entropy` (wrong argument types raise ``TypeError``;
    every other contract violation raises ``ValueError``), ``elements`` is a
    non-empty list of unique non-empty str element names that must all exist
    in ``temporal.data`` (a wrong container or item type raises
    ``TypeError``; an empty, duplicate or unknown element raises
    ``ValueError``), and ``edges`` is a non-empty list of finite non-bool
    int or float bin edges in strictly increasing order; a wrong container or
    item type raises ``TypeError`` while an empty, non-finite or
    non-increasing list raises ``ValueError``.
    ...
    ``-sum(p * log2(p))`` over the bins with ``p > 0`` and
    ``uncertainty`` is ``sqrt(sum(u ** 2)) / n`` over the samples'
    uncertainties.
...
def aggregate_weighted_multi_entropy(
    temporal, elements, regions, windows, weights, edges, *, min_count: int = 1
) -> dict:
    """Aggregate several elements into weighted per-window value entropies at once.

    Combines :func:`aggregate_multi_entropy` and
    :func:`aggregate_weighted_entropy`: ``temporal``, ``elements``,
    ``regions``, ``windows``, ``edges`` and ``min_count`` follow
    :func:`aggregate_multi_entropy` (wrong container/item/argument types
    raise ``TypeError``; every other contract violation raises
    ``ValueError``), and ``weights`` follows
    :func:`aggregate_weighted_window` — a non-empty list with one entry per
    region, in region order, each a dict with exactly the keys ``name`` and
    ...
    For every window (in window order), region (in region order) and element
    (in element order), every non-``missing`` cell of every day of the
    inclusive interval contributes a weighted sample ``(w, v, u)`` of cell
    weight, value and uncertainty.  With ``B = len(edges) + 1`` bins, samples
    are binned exactly as in :func:`aggregate_weighted_histogram`, ``count``
    is the number of samples ``n`` and ``bin_weights[i]`` is the sum of the
    weights of the samples counted in bin ``i``; all ``B`` entries are always
    present.  When ``n`` is below ``min_count``, ``entropy`` and
    ``uncertainty`` are both ``None``.  Otherwise, with
    ``W = sum(bin_weights)`` and ``p_i = bin_weights[i] / W``, ``entropy`` is
    ``-sum(p_i * log2(p_i))`` over the bins with ``p_i > 0`` and
    ``uncertainty`` is ``sqrt(sum((w * u) ** 2)) / W`` over the samples'
    uncertainties.
    ...
    The returned mapping uses the key order ``schema, elements, edges,
    windows, regions, data``; ``schema`` is
    ``climate-grid/weighted-multi-entropy-window-v1``, ``elements``, ``edges``
    and ``windows`` are passed through unchanged and ``regions`` lists the
    region names in input order.  ``data`` is a flat list of rows in
    window-then-region-then-element order; each row uses the key order
    ``window, region, element, count, bin_weights, entropy, uncertainty``.
    ``count`` is an int, ``bin_weights`` holds ``B`` floats and every output
    float is ``round(x, 12)`` with negative zero normalized to ``0.0``.
    Inputs are not modified.
    """
    times, lats, lons, data = _validate_temporal(temporal)
    ...
    return {
        "schema": _WEIGHTED_MULTI_ENTROPY_SCHEMA,
        "elements": elements,
        "edges": edges,
        "windows": windows,
        "regions": [name for name, _ in validated_regions],
