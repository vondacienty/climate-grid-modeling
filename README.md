...
- `climate_grid.regional.aggregate_coverage(temporal, element, regions, windows, *, min_count=1)`：重建序列的按窗口区域覆盖率统计，`temporal`、`element`、`regions`、`windows`、`min_count` 的校验、异常及窗口/区域顺序均沿用 `aggregate_window`，但统计的是格点覆盖情况而非值统计。按窗口序、区域序，`total` 为闭区间内天数与区域格点数之积；`available` 为其中 `status` 非 `missing` 的格点-日数，`observed`、`interpolated` 分别计数，故 `available = observed + interpolated`。`available < min_count` 时 `rate`、`observed_rate`、`interpolated_rate`、`uncertainty` 皆为 `None`，否则依次为 `available/total`、`observed/total`、`interpolated/total` 及 `sqrt(Σu²)/available`（u 为各非 missing 格点-日的 uncertainty）。返回键序 `schema, element, windows, regions, data`，`schema` 为 `climate-grid/coverage-window-v1`，`element` 回显，`windows` 同序回显三键字典，`regions` 为输入顺序的 name 列表；`data` 按窗口序，各项键序 `name, regions`，区域项键序 `name, total, available, observed, interpolated, rate, observed_rate, interpolated_rate, uncertainty`；`total`/`available`/`observed`/`interpolated` 为 int，所有输出浮点经 `round(x, 12)` 处理并将负零归一为 `0.0`；不修改输入。

## 限制

- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口、`attribute` 一个趋势归因入口及 `aggregate`、`aggregate_multi`、`aggregate_window`、`aggregate_exceedance`、`aggregate_quantile`、`aggregate_coverage` 六个区域聚合入口。
