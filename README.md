...
- `climate_grid.regional.aggregate_multi(temporal, elements, regions, *, min_count=1)`：多要素版本的区域逐日聚合。`temporal`、`regions`、`min_count` 的校验及异常沿用 `aggregate`；`elements` 为非空 list，各项为互异的非空 str 且存在于 `temporal.data`，类型错抛 `TypeError`，空 list、重复或未知要素名抛 `ValueError`。统计口径与 `aggregate` 相同：按要素、区域、日期对 `status` 非 `missing` 的格点计数，`count < min_count` 时 `mean, min, max, uncertainty` 皆为 `None`，否则依次为值均值、最小值、最大值及 `sqrt(Σu²)/count`。返回键序 `schema, elements, times, regions, data`，`schema` 为 `climate-grid/regional-multi-v1`，`elements`、`times` 原样返回，`regions` 为输入顺序的 name 列表；`data` 按 name 顺序排列，每项键序 `name, elements`，内层按要素顺序，每项键序 `element, count, mean, min, max, uncertainty` 且五项为等长逐日序列，`count` 为 int；所有输出浮点经 `round(x, 12)` 处理并将负零归一为 `0.0`；不修改输入。

## 限制

- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口、`attribute` 一个趋势归因入口及 `aggregate` 一个区域逐日聚合入口、`aggregate_multi` 一个多要素区域逐日聚合入口。
