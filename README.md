...
- `climate_grid.regional.aggregate_window(temporal, element, regions, windows, *, min_count=1)`：重建序列的分窗口区域聚合，`temporal`、`element`、`regions`、`min_count` 的校验及异常沿用 `aggregate`。`windows` 为非空 list，各项为恰含按序 `name, start, end` 三键的 dict；`name` 为唯一非空 str，`start`/`end` 为有效 `YYYY-MM-DD` 公历日期且均在 `temporal.times` 内、`start <= end`；容器及 `name`/日期类型错抛 `TypeError`，其余窗口约束抛 `ValueError`。按窗口、区域顺序，对闭区间内每日 `status` 非 `missing` 的格点计样本，`count` 为样本总数，少于 `min_count` 时 `mean, min, max, uncertainty` 皆为 `None`，否则依次为样本值的算术均值、最小值、最大值及 `sqrt(Σu²)/count`（u 为各样本 uncertainty）。返回键序 `schema, element, windows, regions, data`，`schema` 为 `climate-grid/window-v1`，`windows` 为输入顺序、各项键序 `name, start, end` 的字典，`regions` 为输入顺序的 name 列表；`data` 按窗口顺序，各项键序 `name, regions`，区域项键序 `name, count, mean, min, max, uncertainty`；`count` 为 int，所有输出浮点经 `round(x, 12)` 处理并将负零归一为 `0.0`；不修改输入。

## 限制

- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口、`attribute` 一个趋势归因入口及 `aggregate`、`aggregate_multi` 两个区域逐日聚合入口、`aggregate_window` 一个分窗口区域聚合入口。
