...
- `climate_grid.attribution.attribute(trends, element, driver)`：要素逐年强度对外部驱动因子的归因。`trends` 须为 `summarize` 的完整返回；`element` 为 `data` 内非空 str 要素名；`driver` 为与 `years` 等长的 list，项为有限非 bool 数或 `None`。以该要素 `intensity` 为 y、`driver` 为 x，仅保留 x、y 均非 `None` 的年份；有效年份少于 2 或 Σ(x-x̄)² 为 0 时抛 `ValueError`。`x̄`、`ȳ` 为保留值的算术均值，b 为保留年份上 y 对 x 的普通最小二乘斜率。返回键序 `schema, years, data`，`schema` 为 `climate-grid/attribution-v1`，`years` 为保留年份，`data` 为等长 list，各项键序 `contribution, uncertainty`，值分别为 `b*(x-x̄)` 与对应年份的原 `uncertainty`；输出浮点经 `round(x, 12)` 处理并将负零归一为 `0.0`；不修改输入。

## 限制

- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口及 `attribute` 一个驱动因子归因入口。
