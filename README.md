...
- `climate_grid.regional.aggregate_variance(temporal, element, regions, windows, *, min_count=1)`：重建序列的按窗口区域方差统计，`temporal`、`element`、`regions`、`windows`、`min_count` 的校验、异常及采样规则均沿用 `aggregate_window`（类型错抛 `TypeError`，其余契约违例抛 `ValueError`）。按窗口序、区域序，将闭区间内每日 `status` 非 `missing` 的格点各收集一个 `(v, u)` 样本（v 为值、u 为 uncertainty），`count` 为样本数 n；`n < min_count` 时 `mean, variance, stddev, uncertainty` 皆为 `None`，否则令 `m = Σv/n`，四项依次为 `m`、`Σ(v-m)²/n`、`sqrt(Σ(v-m)²/n)`、`sqrt(Σu²)/n`。返回键序 `schema, element, windows, regions, data`，`schema` 为 `climate-grid/variance-window-v1`，`element`、`windows` 原样回显，`regions` 为输入顺序的 name 列表；`data` 按窗口序，各项键序 `name, regions`，区域项键序 `name, count, mean, variance, stddev, uncertainty`；`count` 为 int，所有输出浮点经 `round(x, 12)` 处理并将负零归一为 `0.0`；不修改输入。

## 限制

- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口、`attribute` 一个趋势归因入口及 `aggregate`、`aggregate_multi`、`aggregate_window`、`aggregate_exceedance`、`aggregate_quantile`、`aggregate_coverage`、`aggregate_variance` 七个区域聚合入口。
