...
- 命令行除版本查询外没有其他功能。
- 数据来源与更多算法尚未定义；目前提供 `idw_grid` 一个插值算法、`fuse_observations` 一个数据融合函数、`build` 一个多要素数据集构建入口、`reconstruct` 一个逐日时间重建入口、`detect` 一个极端事件检测入口、`summarize` 一个逐年趋势汇总入口、`attribute` 一个趋势归因入口及 `aggregate`、`aggregate_multi` 两个区域逐日聚合入口。
